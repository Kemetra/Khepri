> **PROPOSED AMENDMENT on this branch.** `FR-128`, `FR-129` and the narrowed telemetry exclusion
> are drafted for the owner's decision and are governing only if this branch is merged
> (Constitution II). They are gated on `KHEPRI-DEC-034`, which is proposed in the same branch: if
> that decision is not approved, these requirements have no authority and `W1-09` stays unbuilt.
...
- **FR-127**: Cross-organization, expired, deleted, partial, corrupt, restore and concurrent
  ...
  denial.
- **FR-128** (`W1-09`; gated on `KHEPRI-DEC-034`): An owner MAY pin a dataset version or analysis
  ...
  `AUDIT_ACTIONS`.
- **FR-129** (`W1-09`; gated on `KHEPRI-DEC-034`): A recent-activity view MAY be rendered from the
...
- Any calculation, comparison, aggregation or re-rendering of figures on a workspace surface.
  Comparison is `G4/C1`'s; figures are `RRA-004`'s and `RRA-008`'s; presentation components are
  `RRA-012`'s.
