...
def _validate_authorities(
    ...
    for registry in ("decisions", "families", "specifications"):
        for index, artifact in enumerate(registries.get(registry, [])):
            ...
            _validate_approved_at(label, artifact.get("approved_at"), errors)
...
def _validate_approved_at(label: str, approved_at: object, errors: list[str]) -> None:
    if approved_at in (None, ""):
        return
    if isinstance(approved_at, (date, datetime)):
        return
    if isinstance(approved_at, str):
        try:
            date.fromisoformat(approved_at)
        except ValueError:
            errors.append(f"{label}: approved_at must be an ISO 8601 date")
        return
    errors.append(f"{label}: approved_at must be an ISO 8601 date")
