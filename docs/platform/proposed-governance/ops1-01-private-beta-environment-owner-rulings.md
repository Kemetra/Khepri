...
| PostgreSQL | Managed PostgreSQL 17, single node, 1 vCPU / 2 GiB RAM / 10 GiB storage, PITR and private networking enabled |
| Worker replicas | 1 |

The worker starts materially above the web role because report rendering launches pinned Chromium alongside the analytical workload. The database starts at the smallest single-node tier that supports PITR and private networking, because the bootstrap's purpose is to measure rather than to serve load, and a tier is named rather than left as "suitable" so that `OPS1-02` has one authorized shape to provision instead of an operator choosing a paid shape outside this ruling. These values exist only to bootstrap hosted measurement; they are neither final capacity values nor a beta commitment. After creation, capture the live PostgreSQL minor version and complete the mandatory Spaces compatibility verification. `OPS1-09` then runs on the hosted target and may move the provisional values up or down from evidence. Its final environment evidence must record the resulting sizes and captured PostgreSQL minor, and certification must refuse whenever the live minor differs from the recorded value. `OPS1-05` must later run capacity/soak evidence before external private-beta traffic.
...
The next governance PR must translate the applicable rulings into the DEC-008/DEC-006 successor decisions, **a `KHEPRI-DEC-027` successor**, and a **provisional bootstrap descriptor** carrying section 10's measurement shape — atomically enough that the validator sees one unambiguous active authority. Splitting DEC-027's supersession into a later PR would leave a window in which the restatements read as permission the still-active DEC-027 withholds.

**The final environment descriptor is not that PR's to write.** Section 13 approves it at step 5, from the exploratory measurement taken at step 4, which cannot happen before the environment this PR authorizes exists. Requiring both here would force the next PR either to invent final sizing before measuring anything or to delay the transition that authorizes the measurement. So the next PR authorizes the bootstrap at a provisional shape; the final descriptor and its sizing arrive in the step-5 transition, and the governed certification benchmark runs against that final descriptor.
