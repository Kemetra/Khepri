...
| **U1 Design system** | BLOCKED | Shell primitives exist, and `U1-02`/`U1-04`'s **task dependency** on T1 contracts is satisfied by `#343` — but a satisfied dependency is not authority, and **no active specification governs the files these tasks would touch.** `RRA-011`:69-73 governs the catalog *data* and expressly disclaims the surfaces consuming it, because two active specifications over one path fails closed under `AGENTS.md`. `RRA-010`:30-31 puts `shell.css` and `shell-components.css` **outside** its scope. `RCA-002`:134 excludes *"any change to the `RRA` beta journey, its routes, its templates, or its assets"*, and separately excludes any new icon, typeface, or third-party asset. So the data spec disclaims these files, the journey spec excludes them, and the shell spec excludes them. §18 rule 1 — no product code without active authority — therefore blocks `U1-02` through `U1-07`, and `BLOCKED` is correct per §15: the next task, design included, cannot start. **What unblocks it is a bounded specification allocating the governed data-display component layer**, in the same shape `RRA-011` received the catalog — not an implementation slice. **That specification is drafted as `RRA-012` and is proposed, not active**: it governs the report and evidence templates, a new component stylesheet, and presentation-only adoption in two journey templates, and it authorizes `U1-02` and `U1-04` only. It is `RRA`, not `RCA`, because the surfaces rendering evidence are `rra/rendering/templates/` and `rra/journey/templates/` — an `RCA` authority over them would contradict `RCA-002`:134 on the day it merged. On its merge this row becomes `READY_FOR_IMPLEMENTATION` for those two tasks. `U1-03` (chart grammar), `U1-05` (navigation and filter bars), `U1-06`, and `U1-07` are excluded from `RRA-012` and stay `BLOCKED`. `U1-01` (document the merged primitives) is unaffected, being documentation of what exists |
...
- ~~T1/U1 design may proceed during late CAL1 review.~~ **T1 is merged** (item 9). **U1 is `BLOCKED`, and the phrase "U1 design may proceed" is what needs correcting**: no active specification governs the files `U1-02` through `U1-07` would touch. `RRA-011` disclaims the consuming surfaces, `RRA-010` puts `shell.css`/`shell-components.css` outside its scope, and `RCA-002` excludes changes to the journey's templates and assets. The next U1 step is **a bounded specification allocating the data-display component layer**, which is governance work, not design work under existing authority. **`RRA-012` is that draft, proposed and awaiting merge**; it authorizes `U1-02` and `U1-04` only, and excludes the chart grammar, navigation, and visual-regression tasks, which keep their own gap. `U1-01` — documenting the primitives that already exist — is unaffected.
