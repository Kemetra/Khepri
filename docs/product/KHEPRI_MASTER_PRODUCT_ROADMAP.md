...
| R3 Authentication sessions | READY_FOR_IMPLEMENTATION | `R3-01`…`R3-10` are merged: PRs `#182` (`R3-02`), `#184` (`R3-03`, migration `20260815_0016`), `#185` (`R3-04`), `#186` (`R3-05`), `#187` (`R3-06`), `#188` (`R3-07`), `#190` (`R3-08`), `#183` (`R3-09`), `#191` (`R3-10`). The two owner decisions the design note §9 named as gating `R3-03` were settled before it wrote schema. **Core complete — `R3-11` deferred pending provider admission, not blocked.** Ten of eleven tasks are merged and the program's core is done: the beta authenticates with `FR-002` credentials, so `R3-01`…`R3-08` are the whole of what Khepri needs, and `R3-09`/`R3-10` shipped the provider-neutral composition design and the `IdentityProvider` seam. Nothing downstream waits on `R3-11` — `R8`'s shell depends on `R3-06`, which is merged. **Why not `MERGED`:** §15 permits that status "only with a `main` SHA" and `R3-11` has none, so the row would assert a merge that did not happen — the error `#214` caught when an earlier pass invented `MERGED_EXCEPT_R3-11`. **Why not `BLOCKED`:** §15 reserves that for programs whose next task, design included, cannot start. `R3-11`'s design *is* merged; only the adapter is deferred, and on a governance event rather than an engineering dependency. **The deferral, stated precisely:** `R3-11` needs a provider admitted under `KHEPRI-DEC-018` §5, which admits none (line 135) and is fail-closed — "if any gate is absent, revoked, or unverifiable, no external identity provider is admitted". Clerk is the intended provider and `docs/superpowers/specs/2026-08-14-r3-provider-evaluation-clerk.md` records it as `PENDING vendor evidence` with **no material blocker**: five items outstanding — gates 3 (security controls), 4 (subprocessor list), 6 (per-user deletion scope), 9 (at-rest credential KDF), and 7's written subject-stability confirmation. Four are unobtainable from public documentation and two of those are a *retrieval* failure (`trust.clerk.com` 403 to automated access), not an absent guarantee. Gates 5 and 12 are settled by recorded owner decisions of 2026-08-14; gates 1 and 14 are Khepri's own work inside `R3-11`. **The unblocking event is an owner decision recording evidence against every §5 gate, not a slice** — once it is adopted, `R3-11` starts immediately, since the seam it plugs into is already merged. The seam ships without an adapter behind it by design |
...
| R5 Recovery | READY_FOR_IMPLEMENTATION | **`R5-01` merged at `1c51249` (`#227`)** — `docs/superpowers/specs/2026-08-20-r5-01-account-recovery-design.md` settles the eighteen subjects the slice owed and classifies fifteen adversarial and concurrency cases. Design output only: no production code, no migration. `R5-02` is the next task and it **waits on one owner decision**, not on code — see below. The organizing principle is that *recovery ownership follows credential ownership*, which splits recovery into credential recovery (Khepri's today under `FR-002`; an admitted provider's later) and the Khepri security consequences (session revocation, `can_act`, audit, retention — **Khepri's under every provider**, because `FR-007` is a statement about Khepri's own sessions and no provider can make it true). That boundary is preserved **without** a `RecoveryProvider` port: `KHEPRI-DEC-018` §5 admits no provider and its §6 refuses a provider-switching framework, so `R5-04` and `R5-05` are separate operations composed in one transaction instead — the consequences are reachable without the credential half, and nothing was built early. Three things the note settles rather than defers: one-use is enforced by **destroying** the verifier, per `KHEPRI-DEC-015`'s retention table, not by a `used_at` flag; the guard is a conditional `UPDATE` whose `rowcount` decides, never a read-then-write, and `R5-06` must isolate that statement because a service-level pre-check would **shadow** it and every mutant of the `WHERE` clause would survive — the defect found in `R4-05`; and step 2 of the transaction re-reads the account `FOR UPDATE`, because `R4-05` shipped a pre-revocation snapshot bug of exactly this shape. **Three owner decisions are flagged rather than invented**: the recovery-secret expiry horizon (no artifact fixes a number; recommendation 1 hour — **this one gates `R5-02`**), the delivery vendor (port designed, selection left open the way `KHEPRI-DEC-008` leaves the runtime target open), and whether the purge-versus-completion interleaving is an acceptable residual, by analogy to `R4-01` §8.2 — **not** classified here, because an accepted residual is never invented. Depends on `R3` session revocation, which is merged (`session_persistence.py:94`) |
