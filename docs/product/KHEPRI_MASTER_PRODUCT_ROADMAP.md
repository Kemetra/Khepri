...
| **U1 Design system** | READY_FOR_IMPLEMENTATION | **§15: a program's status is the status of its next actionable task, and that task is `U1-04`'s drawer structure**, which now has an approved bounded plan and its RED tests: `docs/superpowers/plans/2026-09-02-u1-04-evidence-drawer-plan.md` and `tests/test_u1_04_evidence_drawer.py` (ten strict-xfail tests driving the macro through the production environment, plus one guard that no page places the drawer before its supply exists). `U1-02` is **merged** at `#350` (`9e92e73`): the seven `RRA-012` FR-092 components live as macros in `rra/rendering/templates/_components.html.j2`, both report templates render through them, `report.css` carries their rules, `wording.py` their bilingual chrome, and the FR-101 guard is mutation-verified. Three of the seven render in the evidence region rather than on the business page because `presentation-visibility-matrix.md` §A.5 puts citation identifiers and every identity field in tier A -- recorded in the plan's 2026-09-02 status block with three other unattended design calls for owner review. `U1-04`'s catalog **supply** stays deferred to its own artifact: every surface in Scope is a stored artifact, so a catalog read changes the materialization path (three `render_materialized` implementers, three `build_context` callers, `pipeline.py`, both wirings, an Excel renderer with no HTML component). **Journey adoption is unauthorized** -- `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename, so that wiring needs an `RRA-010` slice. `U1-03`, `U1-05`, `U1-06`, `U1-07` are excluded from `RRA-012` and remain `BLOCKED`: no active specification governs their files. `U1-01` is documentation of what exists and is unaffected |
