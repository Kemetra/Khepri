...
already left behind. `R6` read `BLOCKED` while all eight of `R6-01`…`R6-08` were merged, and because
every `R7` task row depends on "R6", a reader taking this table literally concluded R7 could not
proceed. `R7-03` had in fact been available since `R6-04` merged.
...
| R3 Authentication sessions | MERGED_EXCEPT_R3-11 | `R3-01`…`R3-10` are merged: PRs `#182` (`R3-02`), `#184` (`R3-03`, migration `20260815_0016`), `#185` (`R3-04`), `#186` (`R3-05`), `#187` (`R3-06`), `#188` (`R3-07`), `#190` (`R3-08`), `#183` (`R3-09`), `#191` (`R3-10`). The two owner decisions the design note §9 named as gating `R3-03` were settled before it wrote schema. **`R3-11` is the only open task and is correctly blocked**: it needs a provider admitted under `KHEPRI-DEC-018` §5, and §135 of that decision admits none. The `IdentityProvider` seam (`R3-10`) ships without an adapter behind it, by design |
| R4 Invitations | READY_FOR_PLAN | `R4-01` merged (#209). `R4-02`, `R4-03` and `R4-04` are unblocked. **`R4-05` is blocked only on `R4-06`**, which is reordered before it because the purge cascade must precede redemption. The two owner decisions it also waited on were answered 2026-08-18 — `R4-01` §8.2 accepts the issuance-versus-purge residual, and §8.5 reads `FR-019` as the last check. R2's membership operations are merged and stable, and R3 actor resolution is available. Note for `R4-01`: adding a caller-supplied role (`invite(role=...)`) makes `test_no_role_change_operation_accepts_a_role_from_its_caller` fail by design — that test marks the point where role validation stops being optional, since neither the domain nor `rca_membership_events` validates roles today |
...
| R7 Commercial RRA bridge | PARTIAL | R6 is merged, so the dependency is met. `R7-01` (`#201`), `R7-02` (`#203` finding + `#205` migration `20260817_0017`) and `R7-04` (`#207`) are merged. **`R7-02` shipped as the migration only** — `KHEPRI-DEC-020` §3 authorized "the migration and its ORM change" and explicitly withheld the bridge service; see the task disposition under Program R7. **`R7-07` is blocked on governance, not code**: `KHEPRI-DEC-020` §1 re-enacts the admitted shape, so the design is settled, but lifting §3 needs a successor record, which is the owner's to make. **`R7-03` is unblocked now** (depends on R3 and R6, both merged) and needs no governance change. `R7-05` and `R7-06` follow it |
