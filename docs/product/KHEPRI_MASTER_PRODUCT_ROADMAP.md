...
| **U1 Design system** | READY_FOR_PLAN | **§15: a program's status is the status of its next actionable task.** `U1-02` is **implemented** (`feat/u1-02-components`, the PR that carries this row): the seven `RRA-012` FR-092 components live as macros in `rra/rendering/templates/_components.html.j2`, both report templates render through them, `report.css` carries their rules, `wording.py` their bilingual chrome, and the FR-101 guard is mutation-verified. Three of the seven render in the evidence region rather than on the business page because `presentation-visibility-matrix.md` §A.5 puts citation identifiers and every identity field in tier A -- recorded in the plan's 2026-09-02 status block with three other unattended design calls for owner review. The next actionable task is `U1-04`'s drawer **structure**, which has no bounded plan yet, so the program reads `READY_FOR_PLAN`. `U1-04`'s catalog **supply** stays deferred to its own artifact: every surface in Scope is a stored artifact, so a catalog read changes the materialization path (three `render_materialized` implementers, three `build_context` callers, `pipeline.py`, both wirings, an Excel renderer with no HTML component). **Journey adoption is unauthorized** -- `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename, so that wiring needs an `RRA-010` slice. `U1-03`, `U1-05`, `U1-06`, `U1-07` are excluded from `RRA-012` and remain `BLOCKED`: no active specification governs their files. `U1-01` is documentation of what exists and is unaffected |
