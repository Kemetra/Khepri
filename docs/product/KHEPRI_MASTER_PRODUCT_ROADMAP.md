...
| OPS1-09 | Run the hosted benchmark **twice**: first exploratory on the provisional shape, then — after the final environment descriptor and sizing are approved from that measurement — a governed certification run against the final descriptor, reissuing `governance/benchmarks/KHEPRI-BMK-001-sizing.yaml` without the retired broker fields | OPS1-03, CAL1 | no | Final web/worker/DB sizing and environment evidence whose certifying run's digest covers the **final** descriptor, not the provisional one — `KHEPRI-DEC-006`'s digest discipline invalidates the exploratory run once the descriptor changes; `visibility_timeout_seconds`, `message_retention_seconds`, `receive_wait_seconds`, and `max_receive_count` leave the file per `KHEPRI-DEC-008` |
...
12. **OPS1-09:** run the hosted benchmark exploratively on the provisional shape, approve the final environment descriptor and sizing from that measurement, then re-run it as the governed certification against the final descriptor and reissue the sizing authority. It cannot precede `OPS1-03`, which produces the target it measures, and the certifying run must be the one against the final descriptor — `KHEPRI-DEC-006`'s digest discipline invalidates the exploratory run once the descriptor changes.
