...
| **U1 Design system** | READY_FOR_PLAN | Shell primitives exist. **The stated dependency is now satisfied**: `U1-02`'s governed data-display primitives and `U1-04`'s evidence drawer both depend on T1 contracts, and `T1-01`…`T1-05` merged through `#343`. This is the open parallel lane. `U1-03`'s chart grammar depends on CAL1/T1, both landed. `READY_FOR_PLAN` rather than `READY_FOR_IMPLEMENTATION` because no approved bounded plan or RED tests exist yet, per §15 |
