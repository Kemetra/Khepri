...
SV1-01 Semantic-view and governed-query authority
  [OWNER-AUTHORED: T1 and stable C1 contracts are now merged; no active
   specification yet authorizes the SV1 product-code slices]
                 |
                 v
...
## 16. Recommended current status at `b5fa6ce`
...
| SV1 Semantic views | BLOCKED | `T1` and the stable `C1` contracts are merged. The remaining dependency is `SV1-01`: owner-authored semantic-view and governed-query authority. No SV1 product-code slice may start until that specification is active |
...
20. ~~**Activate `G4-02`, `G4-03`, and `G4-04`, then implement `C1-01` through `C1-08`.**~~ **Done.** `RRA-008` carries the active two-population comparison authority, `RCA-005` carries active comparison orchestration, `RRA-006` carries the sibling bundle, and both governing specifications freeze the four contract halves by reference. All eight implementation slices are on `main`; §16 records their SHAs and the three owner readings they carry forward without deciding. The first incomplete critical-path item is now `SV1-01` below.
...
22. **`SV1-01` — activate semantic-view and governed-query authority. This is now the first incomplete item on the critical path.** Its `T1` and stable-`C1` dependencies are merged. The remaining dependency is owner-authored authority: no active specification in `governance/registry.yaml` governs the SV1 product-code slices. Do not schedule `SV1-02` through `SV1-08` until the owner merges that specification.
