...
| D1-10 | Add Arabic/English parity, RTL, accessibility, mobile, visual regression, and refusal-state tests | all above | no | Quality evidence. **MERGED `3dd0746` (`#458`) but this row is NOT closed.** `RCA-008` §Exclusions authorizes only the parity, refusal-state and isolation tests §Verification names; the accessibility-evidence and visual-regression programme "remain `U1-03`, `U1-05`, `U1-06` and `U1-07`'s and need their own authority". **That authority is now active** — `RRA-015` and `RCA-010` — so the remaining halves are schedulable under those documents rather than blocked; the owner's `RRA-010` journey-adoption reading is still open and bears on `/beta` alone. RTL and mobile shipped as parity evidence only, never as a layout programme |
