...
| S1-03 | Select one bounded family. **Discharged in substance by `d753e41` + `74e6a19`, not by a slice of its own**: `S1-02` ranked the candidates and `#463` selected rank 2 inline and named it. No standalone selection slice was opened, and the roadmap records what happened rather than inventing an approval event. **Rank 1 remains unselected** — anchoring `rra_beta_sessions.owner_id` needs the owner to name which shape is admissible (cross-package FK, a CHECK or trigger local to `khepri.rra`, or RLS with a tenant role), and `S1-02` §1 bars a slice from picking by convenience | S1-02 | Approved slice |
...
| S1-05 | Close `#152` only after every classified high-risk record is addressed or explicitly accepted. **OPEN, and rank 1 is why**: `S1-04` closed rank 2, but `rra_beta_sessions.owner_id` (`#432`) is neither addressed nor explicitly accepted, and rank 3 folds into whatever closes it | S1-04 | Closeout |
...
| S1 RRA hardening | IN_IMPLEMENTATION | **No longer triage only: the triage merged and an implementation slice merged past it.** `S1-02` `d753e41` (`#462`) ranked three store seams; `S1-04` `74e6a19` (`#463`) closed rank 2, promoting the `#409` directory guard into `khepri.runtime.private_directory` and calling it at all three unguarded sites. **`S1-05` stays open and `#152` with it**, because rank 1 — `rra_beta_sessions.owner_id` unanchored (`#432`) — is neither addressed nor explicitly accepted. That seam is an owner question, not a scheduling one: a fix crosses the `khepri.rra`/`khepri.rca` metadata-tree boundary, and which of the three shapes is admissible is a construction-boundary reading. Still avoid CAL1 hotspots. **No longer ranks `#231`** — that evidence is delivered and the issue closed, so `S1-02` is store seams only |
...
- ~~S1 triage only may proceed if it does not overlap CAL1 files.~~ **Corrected in place: the triage is done and an implementation slice merged past it.** `S1-02` `d753e41` (`#462`) and `S1-04` `74e6a19` (`#463`) both merged after the critical-path queue exhausted at item 25, which is what this lane is for. **What remains here is not schedulable either**: rank 1 (`#432`) needs the owner to name an admissible shape before any code is written, so `S1-05` and `#152` stay open. The CAL1-overlap condition is spent — CAL1 is merged — and the live condition is the boundary reading, not file contention.
