...
| **M3** | Durable trust workspace beta | Active retention/workspace authority; multiple dataset versions and analyses retained; history, report reopen, deletion, evidence, and metric catalog work. **`M3` is MEASURED and reachable in the same local-only form `M2` holds, with one clause open for an owner reading**, on `main` at `c98e946` against the deployed image: six of seven measured clauses pass, in both languages, deterministically across two runs in four separate organizations. Evidence: `docs/superpowers/plans/2026-09-06-m3-acceptance-evidence.md`. **The open clause is `W1-08`'s Methodology Change Notice**: the diff surface is wired and correctly renders nothing when no governed version differs, which is all a single image pinning one version triple can produce — verified on both runs' live pages, not reasoned from source. Whether "history" requires a *demonstrated* methodology diff or is satisfied by the Passport plus a wired diff surface is the owner's reading; `W1-08`'s own tests (`267c50c`, `#377`) are where the changed-version case is proven. **Unlike `M2`, no governed decision decomposes this gate into numbered conditions** — `KHEPRI-DEC-031` §7 did that for `M2` and has no `M3` counterpart — so the ledger measures this sentence's own six clauses and says so. If the owner would rather `M3` rest on numbered conditions, they are the owner's to author; nothing measured would change. The run is local-stack and **authorizes no external participant**, which still needs the hosted environment deferred by `KHEPRI-DEC-031` §4. One finding outside this gate, since **FIXED**: the Team surface (`RCA-002`/`R8`, not an `M3` clause) **raised** `500` in the deployed image — `wiring.py:497` passed an `InvitationService` where `shell_api.py:152`'s Protocol requires the store's `invitations_for_organization`. `wiring.py` now composes a `ShellInvitations` adapter holding both the `SqlInvitationStore` and the `RcaInvitationService` and implementing `invitations_for_organization` over the store's expiry-aware listing. **A second defect behind it was found reviewing that restatement and fixed in the same PR**: `_team_response` passed `now=None` into that expiry-aware listing, so an organization holding any invitation raised `TypeError` on `stored <= now` -- invisible because every Team test lists an empty organization. The request's instant is now carried on `_Scoped` from the single `clock()` read that resolves the actor. The surface is answerable in the built wheel for an organization that has actually invited someone (`#506`, `#508`) |
| **M4** | Sellable decision workspace | Governed multi-period comparison, curated semantic views, executive overview, branch/product/basket/concentration modules, evidence drawer, and deterministic guided drill-down work. **`M4` is ACCEPTED — 2026-09-16, on `main` at `5c4c522`**, on all seven exit-gate clauses passing against the deployed image. Measured at `44d54a7` as 5/7 with clauses 3 and 4 failing on one shared blocker (`#471`); `#472` (`D1-12`) shipped the single unblocker; re-measurement by navigation found 7/7. Evidence: `docs/superpowers/plans/2026-09-16-m4-acceptance-evidence.md`. **Local-stack, and non-paying by governance** — `KHEPRI-DEC-025` §5's hard stop is unchanged, so acceptance admits no external participant and takes no consideration. Acceptance is scoped to the gate sentence: `D1-08`'s export reading, `D1-11`, `PeriodComparisonView`'s consumption by the decision surface, and `U1`'s design programme all stay exactly as they were |
...
D1-01 Information architecture, narrative order, and fact/view source map
  [DONE at 659b342 -- nine surfaces mapped, narrative order fixed, the
   eight views resolved from the published registry]

D1 product-code authority -- RCA-008 Executive Decision Workspace Surfaces
  [MERGED at 2734886 / #444 -- FR-159..FR-171. Answers D1-01's findings:
   "from SV1 only" governs figures not pages; the period is a source
   selector not a filter; breakdown availability is not synthesizable.
   Defers the Period Comparison source to a successor -- see §17 item 25]

CURRENT CRITICAL-PATH FRONTIER
D1-02..D1-10 implementation
  [AUTHORIZED. Allocation plan at
   docs/superpowers/plans/2026-09-10-d1-02-10-decision-workspace-allocation-plan.md
   D1-11 stays unauthorized: RCA-008 §Retention leaves KHEPRI-DEC-015 §3
   unamended. M4 needs a Compare source RCA-008 does not claim -- item 25]
                 |
                 v
...
**C1-06 retention correction (`#429`).** The C1-06 row above captured the shipping defect rather
than the active authority: `KHEPRI-DEC-033` already keeps the fact package with its run and the
manifest with its dataset version. Workspace-bound session content therefore remains readable with
those records; only the raw upload ends seven days after sealing. Comparisons and report handoffs do
not disappear on day eight. The beta-only session horizon is unchanged.
...
| D1-10 | Add Arabic/English parity, RTL, accessibility, mobile, visual regression, and refusal-state tests | all above | no | Quality evidence. **MERGED `3dd0746` (`#458`), and this row is now CLOSED.** `RCA-008` §Exclusions authorized only the parity, refusal-state and isolation tests §Verification names; the accessibility-evidence and visual-regression programme "remain `U1-03`, `U1-05`, `U1-06` and `U1-07`'s and need their own authority". That authority became active as `RRA-015` and `RCA-010`, and **those halves have now all merged** — shell at `addf010` (`#486`) and `046883c` (`#488`), `RRA` at `9342720`/`692134e` (`#497`/`#498`) and `4469b1c` (`#499`), 2026-09-19. The five floors `#497` recorded failing sit on `RRA-015`'s files, which `RCA-008` does not govern. The owner's `RRA-010` journey-adoption reading is still open and bears on `/beta` alone. RTL and mobile shipped as parity evidence only, never as a layout programme |
...
| D1-12 | Reach the decision surface from the Analysis Passport | D1-03 | no | **The `M4` clause-3/4 unblocker.** One governed entry point, offered per run because the route is addressed per run, guarded on `offers_decisions` (`FR-049`) and on a completed run. Authorized by the owner's reading of `RCA-008` §Scope, which names `shell_api.py` and `shell_templates/`; §Exclusions reserves a navigation *programme*, and one link is not one. The frame's settled `Overview · Data · Analyses · Team` set is unchanged, with a test asserting the entry point never leaks into it |
...
| S1-02 | Identify caller-controlled identifiers and security material at store seams. **Its `#231` clause is discharged**: that evidence was delivered directly rather than waiting on this triage, so rank only the store seams. **MERGED `d753e41` (`#462`)** — `docs/superpowers/plans/2026-09-15-s1-02-store-seam-triage.md` ranks three seams and records four already sealed | S1-01 | Ranked list |
| S1-03 | Select one bounded family. **Discharged in substance by `d753e41` + `74e6a19`, not by a slice of its own**: `S1-02` ranked the candidates and `#463` selected rank 2 inline and named it. No standalone selection slice was opened, and the roadmap records what happened rather than inventing an approval event. **Rank 1's shape is named by `KHEPRI-DEC-035` (proposed on this branch; approved by the owner's merge)**: row-level security with a non-owner runtime role is the only admissible shape; the cross-package FK and a local CHECK/trigger are refused with reasons. No implementation is authorized until a specification requirement lands (Constitution IV) | S1-02 | Approved slice |
| S1-04 | Implement one family with accidental-bypass tests. **MERGED `74e6a19` (`#463`)** for rank 2. **Its extent is wider than the triage ranked, deliberately**: a sweep of `src/` for `mkdir(` found three unguarded sites — `runtime/wiring.py`, `local/wiring.py`, and `local/cli.py`, the last named by no prior note — so the guard moved into `khepri.runtime.private_directory` and the extent test derives from the sweep rather than from the triage, which would leave site four open the day someone adds it | S1-03 | Bounded PRs |
| S1-05 | Close `#152` only after every classified high-risk record is addressed or explicitly accepted. **OPEN, but no longer on rank 1**: `S1-04` closed rank 2, and `KHEPRI-DEC-035` (proposed on this branch; approved by the owner's merge) §3 explicitly accepts rank 1 (`#432`) under `KHEPRI-DEC-031`'s local-only conditions, bounded, with RLS a beta-authorization gate (§4); rank 3 folds into rank 1. `#152` still holds the unscoped `session_id` store lookups `#578` routed to it | S1-04 | Closeout |
...
## 16. Recommended current status at `3a216b6`
...
| R8 Commercial shell | READY_FOR_PLAN | **`R8-08` is AUTHORITY-BLOCKED, not merely unscoped** — it conflicts with active `KHEPRI-DEC-015` and resumes only by amending it (`KHEPRI-DEC-031` §5), so it is not R8's next actionable task. **`R8-10`'s journey half is MERGED** (`#363`, `523991a`, 2026-09-03) under existing `RRA-010`: the report step groups its seven links as *Report and evidence* then *PDF and Excel*, page-language web report first; the data side was already met by the web report's quality summary (`#350`) and the evidence surface's drawer (`#358`), which that step links. **Its shell half is also MERGED** (`e93356c`, `#376`, `W1-06`) — delivered by a `W1` slice rather than an `R8` one, which is why this row read `not started` until 2026-09-15. The row's acceptance sentence is *"user understands what was computed, caveated, and refused before downloading"*, and the shell's Analysis detail surface satisfies it: `shell_analysis.py:60-62` builds the three quality groups from `definitions.summarize` over the bundle rebuilt through the provenance the run binds by digest, and `analysis.html.j2:133-142` renders them in a `quality` region **above** the artifact handoffs at line 144 — so the summary is read before the download controls, which is the ordering the sentence asks for. Bilingual by construction: `quality_summary` is `"Analysis quality"` / `"جودة التحليل"` in `COMPONENT_CHROME` (`wording.py:1647`, `1668`), whose key parity across both governed languages is enforced structurally at `wording.py:1702-1705`. The evidence entry point is present too: `evidence` is one of the four `ARTIFACT_KINDS` handed off (`shell_analysis.py:68-72`). **No `R8-10` slice remains to plan.** This is the drift §15 warns about — a slice that satisfies another program's row has no reason to update it — and it is recorded here rather than left for a later reader to re-derive, because the stale clause caused `R8-10` to be recommended as available implementation work on 2026-09-15 when it was already built. **One finding for the owner came out of it**: every surface, the web report included, is served `Content-Disposition: attachment` (`report_api.py:562`), so no card opens a report in the browser — whether the two HTML surfaces should be served inline is an `RRA-006` reading, filed in the slice's plan. Browser handoff may require successor authority for external partner use; `R8-11` is deferred with the hosted environment |
...
| **U1 Design system** | IN_IMPLEMENTATION | **Both allocation plans are exhausted: the shell family's chain and the `RRA` family's are each complete on `main`.** The program stays `IN_IMPLEMENTATION` rather than moving to `MERGED` because §15 sets a program's status from its **next actionable task**. **Historical rationale, corrected at `#508`:** this read that one such task existed and was unblocked -- the `RRA-015` follow-on slice named for `report.css`, owning the five accessibility floors slice 9a recorded as strict `xfail`s. **That slice merged at `3a216b6` (`#502`)**, so it is no longer this program's next actionable task; what remains is named further down this row and is non-executable rather than schedulable. §15 sets a program's status from its next actionable task, and merged work plus remaining actionable tasks is `IN_IMPLEMENTATION` — the shape `D1`'s row already uses. **Per-slice `main` SHAs (§15 requires one per task):** `U1-01` `457f276` (`#364`, the design-language reconciliation that is this task; the master specification followed at `0d3cf87`, `#475`, and the visual reference pack at `e9726b5`, `#476`); `U1-02` `9e92e73` (`#350`); `U1-03` `b26de23` (`#481`, chart grammar — `RRA` allocation slice 6); `U1-04` `#352`/`#356`/`#358` with the `RRA-011` follow-on at `37455a5` (`#360`); `U1-05` `0d087f3` (`#483`) and `228200c` (`#484`, the state grammar), on the shell type scale and skip-link mechanism merged at `d2a01fc` (`#480`, slice 2b). **`U1-06` and `U1-07` are each split across the two families and BOTH halves of each are now merged**: `U1-06`'s shell half at `addf010` (`#486`, slice 9b) with the 200% overflow fix it found at `0322f7c` (`#487`), and `U1-07`'s shell half at `046883c` (`#488`, slice 10b) with the **partial** typeface fix it found at `7fc4ff9` (`#489`) — which applied the `--font-body` token to every shell and legal page and corrected the member-email overflow that exposed, but **did not** make Noto Sans Arabic arrive: `#489` recorded as an honest limit that the face it names cannot load, and serving those assets is active `RCA-011`'s and is not on `main`. **Those four `RRA`-side tasks are now all merged (2026-09-19):** allocation slice 2, the journey skip-link evidence under active `RRA-010`, at `08e734d` (`#495`); slice 3, the journey type scale, at `535b644` (`#496`); slice 9a, `U1-06`'s `RRA` half under active `RRA-015` `FR-189`, at `9342720` (`#497`) with its post-merge environment-artifact correction at `692134e` (`#498`); and slice 10a, `U1-07`'s `RRA` half under `FR-190`–`FR-191`, at `4469b1c` (`#499`). **Slice 9a shipped its measurement with five floors recorded FAILING as `xfail(strict=True)`, which is the evidence task done correctly, not left undone** — the allocation plan forbids an evidence slice from editing a stylesheet to make its own assertion pass. The five, all confirmed in CI and all on `RRA-015`'s files: the `FR-189` 44px target floor on the report surface's “On this page” anchors at 21px (`report.css`, `en` and `ar`), the refusal panel's floor in `_components.html.j2`, and the evidence surface's ~184px overflow at 390px under 200% text scaling (`en` and `ar`). **That fix slice has since MERGED** at `3a216b6` (`#502`), under `RRA-015` `FR-193` authorized by `RRA-015`'s bounded remediation grant (`b51494f`, `#501`): `tests/test_rra015_report_accessibility.py` records the five floors as fixed with their strict markers removed, for both the `FR-189` target floors and the 200% reflow cases. This row previously read that the fix was *next*, which `#502` falsified the day it landed (`#506`). Under §15 a program's status is the status of its next actionable task, and `U1`'s remaining two — allocation slice 1 and slice 11 — are owner-blocked and asset-dependent respectively, as this row already records below; neither is a §7 task row, so neither is schedulable and the program's implementation path is clear. The `RRA` allocation plan states in terms that the `/beta` journey-adoption reading "bears on the `/beta` surface alone and blocks no slice in this plan", so it is **not** a blocker on the remaining U1 path; it stays the owner's open question. **`D1-10`'s roadmap row therefore CLOSES** — §17 closes it when *those* slices ship, and both halves of both now have. The five floors -- **fixed at `#502`, and recorded failing when this clause was written** -- are on `RRA-015`'s files (`src/khepri/rra/rendering/templates/`), which `RCA-008` — `D1-10`'s authority, governing `src/khepri/rca/`, `shell_api.py`, `shell_templates/` and `shell_assets/` — does not govern, so they were a different program's surfaces and did not hold `D1-10` open even while they failed. The closure stands on both grounds now (`#508`). **What remains of `U1` beyond the fix slice is non-executable and named here so it is not read as schedulable:** allocation **slice 1** (absorbing `#369` into `KHEPRI_DESIGN_LANGUAGE.md`) is **owner-blocked** — master specification §18.3 files it under *Blocked on the owner* and §A.5 records it as a live contradiction; allocation **slice 11** (final polish against the visual reference pack) is **asset-dependent** — §19 gates it on §16 and §16.4 records one reference as absent: #11 narrow/mobile. Neither is a §7 task row, so neither sets this program's status. **The typeface gap `#489` recorded is CLOSED and on `main`:** active `RCA-011` (`056ffc8`, `#490`) authorized the shell to serve its own Noto Sans Arabic faces, and the implementation merged at `64190f7` (`#491`), proving the face loads over a real HTTP origin rather than asserting it — `tests/test_rca011_shell_font_load.py`. **Historical record of how this row read while authority was the open question, kept because it records how the status moved.** **The authority gap is closed; implementation may be planned and may start.** Active `RRA-015` (`FR-181`–`FR-192`) governs `src/khepri/rra/rendering/charts.py`, `_chart.svg.j2`, the chart rules in `report.css` and `report.print.css`, the chart binding points in `html.py` and `wording.py`, and focused test paths — `U1-03`, plus the `RRA` report surfaces' share of `U1-06` and `U1-07`. Active `RCA-010` (`FR-193`–`FR-206`) governs `shell.css`, `shell-components.css`, `workspace.css`, `shell_templates/` and `legal_templates/` as presentation markup and ARIA only, the shell's chrome labels in `shell_copy.py`, and focused test paths — `U1-05`, the shell presentation work including the master specification §19 slice 2b, and the shell surfaces' share of `U1-06` and `U1-07`. **Two artifacts and not one, for a repository rule rather than a change of plan**: `governance/templates/specification.md` requires exactly one family in `depends_on` and `khepri_gov` enforces it, and `RRA-012` §Outcome reads that rule as forbidding one document across two families' surfaces — `U1-03`'s files are `RRA`'s and `U1-05`'s are the shell's. **The two dependencies this row previously named are now one.** The absent specification is supplied; **the owner's `RRA-010` journey-adoption reading remains open and is deliberately untouched** by both artifacts, and it no longer blocks the program, because §15 sets a program's status from its next actionable task and that task — `U1-03` — may start. It stays a named open question against the `/beta` surface only. **This sentence read `READY_FOR_PLAN` and not `MERGED` or `IN_IMPLEMENTATION` when authority had merged but no code had**: "no `U1-03`, `U1-05`, `U1-06` or `U1-07` code exists on `main`". **That is no longer true** — `#480`–`#489` merged `U1-03`, `U1-05`, and the shell half of `U1-06` and `U1-07`, with the per-slice SHAs at the head of this row — so the row moved to `IN_IMPLEMENTATION`. §15 still forbids marking a task complete on anything less than a `main` SHA, which is why `U1-06` and `U1-07` read as halves rather than as done. This row moves off `BLOCKED` the same way `D1`'s did when `RCA-008` merged. **Historical record, kept because it records how the status moved.** It read `BLOCKED`, and the reason follows. **§15: a program's status is the status of its next actionable task.** Everything `RRA-012` and `RRA-013` authorize is **merged** (2026-09-03): `U1-02`'s component layer (`#350`), `U1-04`'s drawer structure (`#352`), active `RRA-013` (`#353`) with its plan and supply (`#354`, `#355`), the drawer placed beside every evidence figure row (`#356`, `#358`), the pagination-test correction that placement required (`#359`), and the `RRA-011` follow-on in which the citation route reads the bundle's evidence and its duplicate projection is deleted (`#360`, `37455a5`). **The "journey adoption" clause this row carried at `#361` was a phantom, and drafting its amendment is what found it.** No `/beta` page renders a governed figure, bundle section state, citation, formula version, or coverage identity — the seven things `RRA-012`'s components present — and every journey state is fetched by JavaScript after the page is served, so the Jinja macros could not render there even if the data existed. An `RRA-010` asset-wiring slice would ship a stylesheet to five pages on which none of its selectors ever matches. The one task that would put bundle state on a journey page is `R8-10`, and its data side is already met: the web report surface opens with the quality summary (`#350`) and the journey's report step links it before the downloads. Filed as `docs/superpowers/plans/2026-09-03-rra010-journey-adoption-reading.md`, **OWNER DECISION, not yet taken**: it recommends no amendment, and until the owner records an answer **the amendment stays on this program's path as pending** — the filing decides nothing, and no slice may act as though option A were chosen. That decision is the owner's and is not a task on this program's path; it is the named dependency that keeps the journey clause closed. What remained of `U1` beyond it was **authority, not code**: `U1-03`, `U1-05`, `U1-06`, `U1-07` had no active specification governing their files. **`U1-01` is delivered**: `KHEPRI_DESIGN_LANGUAGE.md` is reconciled to `f15c835` — §4.11 documents the `RRA-012` component layer, its §0 verification table is re-measured, and §4.5/§8.2/§8.3/§8.5 record the authority that moved. After it, **no `U1` task could start**: `U1-03`, `U1-05`, `U1-06`, `U1-07` awaited a specification and the journey clause awaits the owner's answer to the filing, so per §15 the program read `BLOCKED`, with those two owner-authored artifacts as the named dependencies. **One of the two is now supplied** — see the head of this row |
...
| S1 RRA hardening | IN_IMPLEMENTATION | **No longer triage only: the triage merged and an implementation slice merged past it.** `S1-02` `d753e41` (`#462`) ranked three store seams; `S1-04` `74e6a19` (`#463`) closed rank 2, promoting the `#409` directory guard into `khepri.runtime.private_directory` and calling it at all three unguarded sites. **Rank 1 — `rra_beta_sessions.owner_id` unanchored (`#432`) — is resolved as a governance question by `KHEPRI-DEC-035` (proposed on this branch; approved by the owner's merge)**: RLS is the admissible shape, the interim gap is accepted and bounded, and RLS gates beta authorization. `S1-05` and `#152` stay open only for the unscoped `session_id` lookups `#578` routed to `#152`. Still avoid CAL1 hotspots. **No longer ranks `#231`** — that evidence is delivered and the issue closed, so `S1-02` is store seams only |
| **G2/G3 Workspace authority** | MERGED | **Complete. Both artifacts are active.** `G2-01`'s inventory (`docs/superpowers/specs/2026-09-03-g2-01-retained-data-inventory.md`) measured 22 tables, two object namespaces, two cookies and one telemetry sink on `457f276`. `G2-02` was answered by the owner in session on 2026-09-03: all eight recommended values approved, recorded as the decided table in `KHEPRI-DEC-033` §4 — raw upload purged **7 days** after sealing, deletion evidence **12 months**, **no** inactivity expiry, disabled-organization content frozen then ended at **24 months**, backups **14 days**, owner-only deletion, no export beyond the governed bundle, organization-as-controller. `G2-03` merged that decision (`governance/decisions/KHEPRI-DEC-033-durable-retail-content-retention.md`), which also **closes `KHEPRI-DEC-015` §8's open backup horizon**. `G3-01`..`G3-03` merged as active `RCA-005` (`FR-109`..`FR-127`), and `G3-04`'s bounded plan is `docs/superpowers/plans/2026-09-03-g3-04-workspace-implementation-plan.md`, allocating ten PRs across `W1-01`..`W1-10`. **One obligation travelled with this authority, and is now DISCHARGED**: `KHEPRI-DEC-033` §5 recorded that no retention sweeper had a caller in the shipped image, so every horizon was intent until `W1-07` shipped the sweep with a deployed caller — and no surface could claim automatic expiry before then. `W1-07b` shipped it: `pyproject.toml:54` declares `khepri-retention-sweep = "khepri.runtime.retention_sweep:main"`, which is the caller in the shipped image §5 asks for, and §5 names no cadence obligation. **The `W1` row below is the record**; stated there once rather than twice here, because two rows answering one governed retention question differently is how a reader refuses a surface statement the decision permits (`#506`) |
...
| **D1 Decision workspace** | IN_IMPLEMENTATION | **The allocation is exhausted: `D1-02`…`D1-10` are all on `main`, `D1-12` merged past it, and `D1-11` — the only task left in §7's table that is not on `main` — is unauthorized.** §15's clause is the reason this is not `MERGED`: "a program stays `IN_IMPLEMENTATION` while merged work exists on `main` and a governance question gates the next branch; the gating question is named in the program's row rather than reverting it to `BLOCKED`". The gating question is `D1-11`: `RCA-008` §Retention declines to amend `KHEPRI-DEC-015` §3, so repeat-use telemetry stays prohibited and `D1-11` is unauthorized rather than unscheduled. **Per-slice SHAs (§15 requires a `main` SHA for each):** `D1-02` `e8ae48d` (`#446`), `D1-03` `2096ab4` (`#447`), `D1-04` `a65df40` (`#448`), `D1-05` `b28b97e` (`#451`, superseding the drawer merged at `#450`), `D1-06` `9f570bc` (`#452`, a plan slice: S-7 already shipped and S-8 is owner-blocked, so it had no deliverable), `D1-07` `79f2d54` (`#454`), `D1-08` `7d91b20` (`#455`), `D1-09` `fe64bd2` (`#457`), `D1-10` `3dd0746` (`#458`), `D1-12` `5c4c522` (`#472`, added to §7 after the original allocation: the `M4` clause-3/4 unblocker). The allocation plan is `docs/superpowers/plans/2026-09-10-d1-02-10-decision-workspace-allocation-plan.md`; each slice's evidence ledger sits beside it. **The Period Comparison authority is resolved and Compare is IMPLEMENTED.** Active `RCA-009` succeeds the collision between `RCA-007`'s composition root and `RCA-008`'s refusal for this binding only, and `FR-180` authorized relocating the comparison operand derivation into one shared runtime seam. **The reachability slice merged at `44d54a7` (`#470`), 2026-09-16:** the composition branch ships and every `FR-170` absence assertion is removed from `src/`, in the same slice, as `FR-179` requires. `D1-03` no longer ships that assertion. **Item 26 is DONE.** The `M4` acceptance measurement of 2026-09-16 (`docs/superpowers/plans/2026-09-16-m4-acceptance-evidence.md`) further records that M4's "compare governed periods" clause is met on the deployed image through the route-reachable `C1` comparison surface, and that `PeriodComparisonView` — reachable through the composition root — is consumed by no route, since `shell_controls.SURFACE_VIEWS` is "seven and not eight" by design and `MetricCard.comparison` stays `None` because no read model fills it. Whether the decision surface should consume it remains an `RCA-008` reading and is not an M4 blocker. **Two owner-authored items remain carried forward.** (1) **The export reading in `D1-08` §Exclusions** — whether "no cross-organization access, sharing, or export" reaches "export". Print shipped; export is undecided. (2) **`D1-11`**, above. **The row this program could not close is now closed by `U1`.** `D1-10`'s own task row in §7 is wider than `RCA-008` §Exclusions authorizes: the accessibility-evidence and visual-regression halves remained `U1-03`/`U1-05`/`U1-06`/`U1-07`'s and needed their own authority, which active `RRA-015` and `RCA-010` supplied. **Those slices have now all shipped** — the shell halves at `addf010` (`#486`) and `046883c` (`#488`), the `RRA` halves at `9342720`/`692134e` (`#497`/`#498`) and `4469b1c` (`#499`) — **so `D1-10`'s row closes.** `U1` is `IN_IMPLEMENTATION`, its next actionable task being the `RRA-015` `report.css` fix slice, which is on `RRA`'s surfaces and not on `RCA-008`'s. Parity, refusal states and cross-organization isolation are done; RTL and mobile shipped as parity evidence, not as a layout programme. **Why this still differs from `SV1`'s `MERGED`.** `SV1` reached `MERGED` with its composition slice shipped (`RCA-007` at `2337cd4`); `RCA-009` grants the analogous authority but its composition slice has not shipped. **Historical note, kept because it records how the status moved.** It read `READY_FOR_PLAN` at `#444`: "the authority merged at `2734886`, so §15 moves this row off `BLOCKED`: the program's next actionable task is `D1-02`, and it may start." Active `RCA-008` (`FR-159`–`FR-171`) governs `src/khepri/rca/workspace/`, `shell_api.py`, `shell_templates/` and `src/khepri/runtime/shell_assets/`, for the D1 surfaces only. Before that it read `BLOCKED`, naming the absent product-code authority as the dependency; `D1-01` (`8e6dc94`, `#440`) was the information-architecture note that could proceed meanwhile |
...
23. ~~**`D1-01` — define the information architecture, narrative order, and exact fact/view source map.**~~ **Done** — `docs/superpowers/plans/2026-09-10-d1-01-workspace-information-architecture.md`, on `main` at `659b342`. It maps all nine D1 surfaces to their sources, fixes the narrative order on one rule (*a claim may be shown only after the reader can see whether it is allowed to be made*), and resolves the eight views' metrics, dimensions, filters, output order and empty rules from the published registry rather than transcribing them. **Three findings came out of it, each verified against the shipping code and none of them decided here** — they are §9's items for the owner and are summarized in item 24. The original scheduling note is kept below because item 24 inherits its constraints.
    ...
    **The owner-authored artifacts this queue may not schedule** are the D1 product-code authority above and the `RRA-010` journey-adoption reading (`docs/superpowers/plans/2026-09-03-rra010-journey-adoption-reading.md`), which is still open. **It no longer keeps `U1` `BLOCKED`** — active `RRA-015` and `RCA-010` supply the specification `U1` lacked, and the reading now bears on the `/beta` surface alone. `D1-01` may name the components it wants; it may not assume a stylesheet can reach a page.

24. ~~**Activate the D1 product-code authority, then implement `D1-02` through `D1-11`.**~~ **Half done — the authority is active.** `RCA-008` Executive Decision Workspace Surfaces merged at `2734886` (`#444`), carrying `FR-159`–`FR-171`, and it answers all three decisions `D1-01` handed it: `FR-159` reads "from SV1 only" as a rule about **figures, not pages** (admitting all nine surfaces); `FR-166` models the global period as a **source selector, not a filter**; `FR-167` bars synthesizing a breakdown's four-state availability from the core metric it aggregates. **The implementation half is item 25.** The original text is kept below because item 25 inherits its constraints.

    **What it did not claim, deliberately.** The cross-version binding `D1-01` recommended admitting here was **declined**: it lives in `semantic_view_adapter.py`, which `RCA-007` §Scope already governs, and two active specifications over one file is the ambiguity this repository fails closed on. `FR-170` holds the Period Comparison surface open visibly instead. That is the M4 gap in item 25.

    **Superseded framing, kept verbatim:** No active specification in `governance/registry.yaml` governs the D1 product-code files: `RRA-014` puts "runtime routes, templates, shell assets, workspace persistence" outside its Scope, `RCA-006` excludes "no route, runtime wiring, template, shell asset, public API, migration, cache, background job, renderer, or existing workspace file", and `RCA-007` excludes routes and HTTP responses by `FR-158`, asserted in test. **D1 is made entirely of what all three exclude.** Do not schedule `D1-02` through `D1-11` until the owner merges that specification — the same instruction item 22 recorded holding for `SV1-02`…`SV1-08`, and it held.

    **`D1-01` hands that authority three decisions, none of which this queue may take.** They are the note's §9 and are named here so the specification does not have to rediscover them.

    1. **`D1-02`'s "from SV1 only" needs one sentence of reading.** Three of the nine surfaces are not `SV1` reads — Recent Analyses and Comparisons (`W1` records and pins), the Navigable Report Workspace (`RRA-006` bundle, `W1` detail), and the definition half of the evidence drawer (`RRA-011`'s catalog). **Recent Analyses cannot be an `SV1` read at any view version**, because `ScopedSourceReader.get_analysis_run` reads one run by id and no published projection ranges over runs. The note recommends reading the rule as *no figure is derived outside an SV1 projection*, which admits all nine; read as *every surface is an SV1 read*, one surface is unbuildable.
    2. **`PeriodComparisonView` is published and unreachable on the shipping composition root.** It accepts `two_population_bundle`; `semantic_view_adapter._rebuilt_bundle` returns `ReportBundle.of(package)` and nothing else, so two `source_ids` yield two single-population bundles. The shape predicate reads `bundle_version`, and `rra006.bundle.v8` carries no `crossversion` marker — so `FR-136` refuses the view. The only `assemble_crossversion` caller in the runtime is `comparison_assembly.py`, which is not bound to the semantic-view port. **This is not a defect in `SV1` or `RCA-007`**: neither admits that composition, exactly as `RCA-006` admitted no composition root until `RCA-007` existed. The note names three options and recommends admitting the cross-version binding in the D1 authority, reusing the existing assembly. Until then, `SV1`'s absence-marker discipline is the precedent for holding it open honestly; a Compare tab that renders empty has hidden the gap.
    3. **`D1-07`'s "global period filter" is not a filter.** No published view names `period` in `request_filter_allowlist` and five name no request filter at all; a period is a property of the run's package, so choosing one is choosing a `source_id`. `organization_id` is resolved by `resolve_scope`, not filtered. Only `store`, `product` and `category` are real view filters, on three views. Because `FR-137` refuses an unsupported filter *before* projection rather than dropping it, a filter bar that passes a period into `ExecutiveOverviewView` **gets a refusal, not an unfiltered overview**.

    4. **`MetricAvailabilityView` cannot qualify a single figure on Branch Performance or Product/Category Performance.** It publishes 22 metrics; the source map projects 28 across those surfaces, and the six it omits are exactly the series metrics (`revenue_by_store`, `units_by_store`, `revenue_by_product`, `units_by_product`, `revenue_by_category`, `units_by_category`) that S-3 and S-4 are entirely made of. Deliberate rather than an omission — `registry.py`'s `_series` composes only over `SERIES_MEASURES`, and the view selects governed contracts rather than series compositions — but it means the Exceptions surface cannot state the four-state card status for a breakdown figure. Extending it is an `RRA-014` change and a **new view version** under `FR-134`, never an edit. **No slice may synthesize a breakdown's availability from the core metric it aggregates**; that is a derivation and `FR-138` bars it.

    **One performance obligation travels with this item.** `SV1-08` ledger §3a measures the composed request at 4003 µs p50 with projection at 11.6 µs — one part in 345 — so `D1-09`'s target is source acquisition, and `FR-144` bars caching, pre-aggregation, materialized views and sampling from that path, making any change there governed rather than local. A card that issues four view reads (the note's §5) multiplies acquisition, not projection.

25. ~~**Implement `D1-02` through `D1-10`.**~~ **Done; the D1 allocation is exhausted.** All nine slices are on `main` in numeric order: `e8ae48d` (`#446`), `2096ab4` (`#447`), `a65df40` (`#448`), `b28b97e` (`#451`), `9f570bc` (`#452`), `79f2d54` (`#454`), `7d91b20` (`#455`), `fe64bd2` (`#457`), `3dd0746` (`#458`). §16's D1 row carries the per-slice SHAs and reads `IN_IMPLEMENTATION` rather than `MERGED`, because `D1-11` remains in §7's table and is unauthorized. The original text is kept below because it records the constraints the slices were built under.

    **The former absence of item 26 was a finding, not an omission.** At that point the next move was owner-authored because Period Comparison had no composition authority. `RCA-009` resolves that authority question, so item 26 is now actionable while Compare itself remains unimplemented.

    **The first critical-path gate is resolved; the remaining owner questions do not block M4:**

    1. ~~**The successor composition artifact for Period Comparison.**~~ **Resolved by active `RCA-009`.** The owner selected the new RCA successor shape and ruled that M4 includes Compare. `RCA-009` explicitly succeeds the `RCA-007` / `RCA-008` collision for this binding, so the remaining M4 gate is implementation, not authority. `FR-170`'s asserted absence stays in place until the reachability implementation removes it in the same slice.
    2. ~~**`U1`'s `RRA-010` journey-adoption reading.**~~ **The authority half is resolved.** `U1-03`/`U1-05`/`U1-06`/`U1-07` — the accessibility-evidence and visual-regression programme `RCA-008` §Exclusions explicitly leaves to them — are authorized by active `RRA-015` and `RCA-010`, so `U1` reads `READY_FOR_PLAN` and `D1-10`'s row can close once those slices ship. The `RRA-010` journey-adoption reading is still the owner's and still open; it now bears only on the `/beta` surface and blocks no program.
    3. **The export reading in `D1-08` §Exclusions**, and **`D1-11`**'s telemetry prohibition. Both are narrow, and neither blocks a milestone; they are recorded so a later reader does not mistake them for oversights.

    **Do not open a D1 implementation slice.** The allocation plan allocated thirteen requirements to nine slices and all nine have merged; there is no tenth to plan.

    **Superseded framing, kept verbatim:** Active `RCA-008` governs the files, and the allocation plan is `docs/superpowers/plans/2026-09-10-d1-02-10-decision-workspace-allocation-plan.md` (the `G3-04`/`SV1` tier: it allocates thirteen requirements to nine slices and writes no code). Build order is numeric; each slice opens its own execution plan.

    **`U1` is no longer in the way, and that is `RCA-008`'s doing.** Precondition 3 makes `U1`'s chart grammar, navigation, accessibility and visual-regression tasks **not preconditions** for D1, and lets a slice needing a decision-surface stylesheet rule take it from `RCA-008` §Scope. So these nine slices proceeded while `U1` waited. **`U1` now has its own authority** — active `RRA-015` and `RCA-010` — and `RCA-008` is unchanged by it: a `D1` slice still takes a decision-surface rule from `RCA-008` §Scope, and `RCA-010` §Successor reading item 5 succeeds `RCA-008`'s `shell_assets/` claim only for presentation rules shared across shell surfaces.

    **Two roadmap rows are wider than the authority, and a slice may not close the gap by choosing the convenient reading.**

    1. **`D1-03`'s "change summary" has no reachable source.** The summary is the Period Comparison surface, and `FR-170` requires it be held open visibly and *asserted* unreachable rather than rendered as an empty tab. `D1-03` therefore ships the KPIs plus that assertion, which is removed by the slice that makes the source reachable — not by `D1-08` or `D1-10`.
    2. **`D1-10` does not close its own roadmap row.** `RCA-008` §Exclusions authorizes the parity, refusal-state and isolation tests its §Verification names, and stops there: the accessibility-evidence and visual-regression programme "remain `U1-03`, `U1-05`, `U1-06` and `U1-07`'s and need their own authority." **That authority now exists** — active `RRA-015` and `RCA-010` — so the row closes when those slices ship, not when this one did. No `RCA-008` requirement changed."

    **One thing the plan raises and does not decide:** `D1-08`'s roadmap row says "print/export/snapshot", but §Retention forbids a stored snapshot and §Exclusions bars "no cross-organization access, sharing, or export" — whether that modifier reaches "export" is not resolvable from the document. Print is in scope; export is an owner question.

    **The M4 consequence, stated rather than buried.** D1 alone does not satisfy M4's "compare governed periods." Active `RCA-009` resolved the authority precondition, and the reachability slice **merged at `44d54a7` (`#470`)**. **Updated 2026-09-16 by the `M4` acceptance measurement** (`docs/superpowers/plans/2026-09-16-m4-acceptance-evidence.md`): M4's "compare governed periods" clause is **met on the deployed image**, satisfied by the route-reachable `C1` comparison surface — an admitted `CrossVersionBundle` over two differing governed periods, 20 cited bilingual figures, discoverable as a form on Analyses. `C1`'s comparison is a period comparison by its own contract (`dataset_period.py`: "February against March is the ordinary case of this family"; `periods_comparable` never refuses because two periods differ). **There is no M4-without-Compare branch, and none is needed.** The clause that now blocks M4 is a different one — see item 26.

    **`D1-11` is unauthorized, not unscheduled.** `RCA-008` §Retention: `KHEPRI-DEC-015` §3's prohibition on repeat-use telemetry "is not amended here and `D1-11` stays unauthorized."

26. ~~**Plan and implement the `RCA-009` Period Comparison composition slice.**~~ **DONE — merged at `44d54a7` (`#470`), 2026-09-16.** One bounded slice bound the existing governed two-population source to the exact `PeriodComparisonView` path in `semantic_view_adapter.py`, preserving fail-closed compatibility and all existing isolation, provenance, bilingual, calculation, no-write, and no-event rules, and removing `FR-170`'s asserted absence in the same slice as `FR-179` requires. It added no persistence, migration, cache, pre-aggregation, materialized view, sampling, telemetry, route, view definition, or broader runtime redesign. **History, kept because it records how the item moved.** It read "the authority is active; Compare is not implemented … ready for a bounded implementation plan, not complete" until `#470` shipped. **The authority blocker this item hit was resolved before that.** A first implementation attempt found `RCA-009` §Scope's exclusion of `comparison_assembly.py` blocking rather than bounding: `DatasetPeriod.complete` is the coverage manifest's answer, and the only derivation establishing it is that module's private operand loader, so the composition branch could reach the governed source only by reimplementing it in the adapter. `RCA-009` `FR-180` now authorizes the narrow alternative — mechanically relocating that derivation into one shared runtime seam both callers use — with no second implementation and no redesign of the module. `FR-180` is the authority `#470` built on.

    **The next M4 action is not this item.** The `M4` acceptance measurement of 2026-09-16 (`docs/superpowers/plans/2026-09-16-m4-acceptance-evidence.md`, run on `44d54a7` against the rebuilt staging image) reported **five of seven `M4` clauses PASS and two FAIL** at `44d54a7`. The two failing clauses were "view an executive decision page" and "drill through supported breakdowns", failing together for one reason: **no shipped surface linked to `/app/{language}/{organization}/decisions/{source}`**, whose `{source}` segment needs a `run_id` no surface exposed. The page rendered fully and bilingually with all four breakdown sections and working filters — it was unreachable, not unbuilt.

    **That blocker is cleared.** The owner ruled that `RCA-008` authorizes one entry point — `RCA-008` §Scope already names `shell_api.py` and `shell_templates/`, and §Exclusions reserves a navigation *programme*, which one link is not. **`D1-12`** ships it on the Analysis Passport, offered per run because the route is addressed per run, guarded on `offers_decisions` (`FR-049`) and on a completed run, and leaving the settled `Overview · Data · Analyses · Team` frame set untouched with a test asserting it never leaks in. Re-measured by navigation on a rebuilt image — Analyses → Passport → decision surface, following only rendered links, no address composed by the harness — **all seven `M4` clauses now PASS in both languages**. See the addendum in `docs/superpowers/plans/2026-09-16-m4-acceptance-evidence.md`. **`M4` is ACCEPTED on that evidence — 2026-09-16, on `main` at `5c4c522`** (`D1-12` merged at `#472`). Acceptance is scoped to the gate sentence; the recorded caveats are unchanged and none of them is closed by it. See §M4 exit gate.

27. ~~**The `U1` design-system chain — both allocation plans.**~~ **Done; both are exhausted.** The shell family completed at `046883c` (`#488`) with the partial typeface fix at `7fc4ff9` (`#489`) and the assets themselves at `64190f7` (`#491`) under active `RCA-011`. The `RRA` family completed 2026-09-19: slice 2 `08e734d` (`#495`), slice 3 `535b644` (`#496`), slice 9a `9342720` (`#497`) with its post-merge correction `692134e` (`#498`), slice 10a `4469b1c` (`#499`). `FND-005` (`8730a4c`/`ce94f9b`/`79a0557`, `#492`–`#494`) made CI actually run the browser contracts it had been reporting green on while installing no browser. **`D1-10`'s roadmap row closes here.** Two allocation slices remain and neither is executable: **slice 1** is owner-blocked (master specification §18.3, §A.5) and **slice 11** is asset-dependent (§19 gates it on §16; §16.4 records one reference absent: #11 narrow/mobile).

28. ~~**The `RRA-015` `report.css` accessibility-floor fix.**~~ **Done — all five floors hold and every strict marker is deleted, which `FR-193` makes the completion test.** Two of the three recorded reasons were wrong: `min-block-size` does nothing to a non-replaced inline box, so the anchors needed `inline-flex` before the floor could apply, the Arabic surface also failed on width, and the skip link was a third failing tab stop no reason named; and the reflow was **not** the figures table, which its own `.scroller` contains, but `dl.provenance`, whose `minmax(12rem, auto)` term column doubles past its box at 200% text. **A plausible subject named in an `xfail` reason is a hypothesis, not a diagnosis.** **How it stood before this item closed, kept because it records what the remediation was measured against.** Slice 9a measured `FR-189`'s floors and, as its allocation plan requires, **recorded** five failures rather than fixing them: an evidence slice that edits a stylesheet to make its own assertion pass has left its scope. All five were live as `xfail(strict=True)` and all confirmed in CI, until this item deleted them. **This queue previously said “no new authority is needed”, and that was wrong** — `RRA-015` §Scope governed `report.css` for *chart presentation rules only*, §Successor reading item 3 said “no slice under this document changes a component rule”, and `_components.html.j2` was named by neither `RRA-015` nor `RRA-012` except by directory. All five failures sit in component rules, so the document could measure floors it could not satisfy. **A bounded amendment closes exactly that gap**, adding `FR-193` and the §Successor reading item 3a carve-out for these five recorded failures alone; `RRA-012` keeps the component layer generally. **That amendment merged at `b51494f` (`#501`), so the authority is in place.** They were the 44px target floor on the report surface's “On this page” anchors, which rendered at 21px in both languages (`report.css` declared no minimum-target rule at all); the refusal panel's floor in `_components.html.j2`; and the evidence surface's ~184px horizontal overflow at a 390px viewport under 200% text scaling, in both languages. The strict markers failed the day each floor was met, which is what made the fix slice self-verifying and what `FR-193` turned into its completion test. **Proving the floors under the shipped Arabic faces needs an HTTP origin so `@font-face` fetches** — `tests/test_rca011_shell_font_load.py` is the worked example; a `set_content` page has no origin and measures whatever the host has installed.

29. **`OPS1-02`, then `OPS1-03` — executable, and the next track once item 28 lands.** `KHEPRI-DEC-030` §3–4 unblocks the provisional bootstrap shape and `KHEPRI-DEC-031` §4's deferral ran past `M2`, which was accepted 2026-09-16. It is **deferred, not blocked**: the authority is active and unexercised, so it resumes by scheduling. Per §15 it is `READY_FOR_PLAN` and not `READY_FOR_IMPLEMENTATION` — no approved bounded plan or RED tests exist yet. Items 12–14 above carry the detail.
...
- ~~T1/U1 design may proceed during late CAL1 review.~~ **T1 is merged** (item 9). **U1 is `IN_IMPLEMENTATION` (§16), and this bullet records how it got there rather than disputing it.** It went `BLOCKED` → `READY_FOR_PLAN` → `BLOCKED` again, and the phrase "U1 design may proceed" was wrong for the first of those: when this entry was corrected, no active specification governed the files `U1-02` through `U1-07` would touch. `RRA-011` disclaims the consuming surfaces, `RRA-010` puts `shell.css`/`shell-components.css` outside its scope, and `RCA-002` excludes changes to the journey's templates and assets. That gap was then closed for `U1-02` and for the presentation half of `U1-04`: **active `RRA-012` allocates the data-display component layer**, so U1 moved from `BLOCKED` to `READY_FOR_PLAN` — design and a bounded plan could proceed, and implementation would begin when that plan's RED tests existed. **That readiness was then spent, and `BLOCKED` was the status for a time — this bullet is not a competing claim about the current one, which §16 carries.** `U1-02` merged at `#350` and `U1-04` at `#352`/`#356`/`#358` (2026-09-03), which consumed exactly the work `RRA-012` authorized; §15 then sets the program's status from its *next* actionable task, and every remaining one is authority-blocked (below). Read this sentence as the record of a transition that completed, not as a live state. Supplying the drawer's catalog data is deferred to its own artifact, because it is a materialization-path change rather than a component one. It excludes the chart grammar (`U1-03`), navigation (`U1-05`), accessibility evidence (`U1-06`), and visual-regression (`U1-07`) tasks, which kept the original gap and stayed `BLOCKED` — `RRA-012` §Implementation preconditions says so in terms ("`U1-03`, `U1-05`, `U1-06`, and `U1-07` are **not** authorized here"), and `RCA-008` §Exclusions repeats that they "need their own authority". **That authority has since been written**: active `RRA-015` carries `U1-03` and the `RRA` report surfaces' share of `U1-06`/`U1-07`, and active `RCA-010` carries `U1-05`, the shell presentation work, and the shell surfaces' share of `U1-06`/`U1-07`. Both statements above remain accurate readings of `RRA-012` and `RCA-008` alone; neither document changed. **It also does not authorize component adoption on the journey surface** — the stylesheet cannot reach a journey page without an `RRA-010` asset-wiring slice, and whether that slice is warranted is the owner's `RRA-010` journey-adoption reading, filed and **not yet taken**. `U1-01` — documenting the primitives that already exist — is delivered and unaffected. **So U1's next actionable task once needed authority nobody had written. It is written, and the work has since completed: `U1-03` merged at `b26de23` (`#481`), and the `RRA` allocation plan's slices 2 and 3 under `RRA-010` and 9a and 10a under `RRA-015` all merged through `#499` (2026-09-19), exhausting both chains — see item 27. The program still reads `IN_IMPLEMENTATION` as §16 records, and its next actionable task is now item 28, the `RRA-015` `report.css` accessibility-floor fix.** The `RRA-010` journey-adoption reading stays the owner's, stays open, and bears on the `/beta` surface alone.
...
- ~~S1 triage only may proceed if it does not overlap CAL1 files.~~ **Corrected in place: the triage is done and an implementation slice merged past it.** `S1-02` `d753e41` (`#462`) and `S1-04` `74e6a19` (`#463`) both merged after the critical-path queue exhausted at item 25, which is what this lane is for. **Rank 1 (`#432`) is no longer an open owner question**: `KHEPRI-DEC-035` (proposed on this branch; approved by the owner's merge) names RLS as the admissible shape and accepts the interim gap; implementation waits on a specification requirement and gates beta authorization. The CAL1-overlap condition is spent — CAL1 is merged — and the live condition is the boundary reading, not file contention.
...
- ~~D1 dashboard;~~ **corrected in place, three times. `D1-02`…`D1-10` are now authorized** — `RCA-008` merged at `2734886` (`#444`), so the prohibition this bullet carried is spent for those nine slices. **What still stands is `D1-11`**, which `RCA-008` §Retention leaves unauthorized by declining to amend `KHEPRI-DEC-015` §3. Prior text: **`D1-01` is now done** (`659b342`…, item 23), having read *may start* while it was the open frontier. What stays barred is D1 *implementation* — `D1-02` through `D1-11` — until the owner merges an active specification governing the D1 product-code files. Writing a read model, a KPI module, or a filter surface before then is the prohibition this line still carries;
- billing;
- agency;
- connectors;
- embedding;
