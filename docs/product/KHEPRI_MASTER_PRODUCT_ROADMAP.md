...
| `V-formula` core formulas and refusal rules | `rra004.formula.v2` | `RRA-004` | CAL1-05b, CAL1-07a, CAL1-09a, CAL1-10a, **and the family compatibility gate** |
...
**Ordering alone is not sufficient, and `V-formula` carries a fail-closed gate for the window it opens.** The four families do not check what they are consuming: `bundle._FAMILIES` dispatches `comparison.derive`, `growth.derive`, `basket.derive` and `concentration.derive` unconditionally, and each stamps its own constant — `rra008.comparison.v1`, `rra008.growth.v1`, `rra008.basket.v1`, `rra008.concentration.v1` — with no comparison against the package's `formula_version`. Between `V-formula` merging and each family successor merging, `main` would therefore compute with successor formula semantics and publish under a **predecessor family identity**, which `RRA-004` forbids: "A new input, mapping, formula, population, interpretation, correction, or serialized shape creates a new recorded version and stable identity."

`V-formula` therefore ships a compatibility check in the same slice: each family compares the formula identity it is handed against the family version it stamps, and **refuses its whole family when the formula is newer**, with a governed reason code and complete accepted Arabic and English wording. The consequence is deliberate and recorded rather than hidden — **between `V-formula` and `V-concentration`, the four derived families refuse on `main`** — because a reasoned refusal is what this product offers in place of a plausible number under a stale version. Each family slice's definition of done includes opening its own gate. An implementer who finds the window unacceptable must not remove the gate; co-landing `V-formula` with all four family slices is the alternative, and it needs owner approval because it changes the reviewable unit.
...
| CAL1-05c | The family compatibility gate: each `RRA-008` family refuses while the formula version it consumes is newer than the family version it stamps, with governed bilingual wording | `V-formula` | `V-package` merged |
