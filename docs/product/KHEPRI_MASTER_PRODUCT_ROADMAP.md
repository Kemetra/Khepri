...
- Where an admitted external identity provider authenticates the actor, the same constraints hold
  ...
  authorization decision (`KHEPRI-DEC-024` §4). Provider identity replaces credential verification,
  never authority.

### Task disposition — staged Clerk admission (recorded 2026-08-21)

`KHEPRI-DEC-024` supersedes `KHEPRI-DEC-018` without rewriting the historical `R3-09` design. It
provisionally admits Clerk only for an invite-only, non-paying private beta and preserves all sixteen
evidence gates as a hard commercial stop. The identity seam and session composition remain as
merged: Clerk proves the subject, the local link resolves a Khepri `account_id`, and Khepri holds the
revocable active-organization session used by the live authorization chain.

The admission does **not** make `R3-11` the next safe code change by itself. Repository evidence
shows that `Account.create` requires a local credential and the final-owner query counts only
`credential_digest IS NOT NULL`; an external-only owner would therefore be uncreatable or treated
as unable to authenticate. The next work is a provider-neutral design and persistence probe for an
atomic account-and-link path and effective-owner capability. The companion
...
for local credentials. `KHEPRI-DEC-024` accepts that cross-system residual only for non-paying
private beta and keeps it open for commercial admission. Details and the evidence boundary are in
`docs/superpowers/specs/2026-08-21-clerk-first-identity-and-r5-disposition.md`.
...
| R3 Authentication sessions | READY_FOR_PLAN | `R3-01`…`R3-10` are merged; the provider-neutral session, live-authority chain, external-identity link, and `IdentityProvider` seam exist. Proposed `KHEPRI-DEC-024` provisionally admits Clerk for non-paying private beta, so provider admission is no longer the next design question. Repository evidence instead shows that external-only account creation and final-owner eligibility are not ready: `Account.create` requires a local credential and the effective-owner SQL predicate requires `credential_digest IS NOT NULL`. The next actionable work is the provider-neutral account-and-link design and persistence probe described in `docs/superpowers/specs/2026-08-21-clerk-first-identity-and-r5-disposition.md`; it must settle atomic creation, authentication capability, and whether persistence changes are required before `R3-11` is implemented. The program is `READY_FOR_PLAN`, not `READY_FOR_IMPLEMENTATION`, because no approved bounded implementation plan or migration finding exists |
...
| R5 Recovery | READY_FOR_PLAN | `R5-01` merged at `1c51249` (`#227`) and remains the design for a future Khepri-credential path. Under proposed `KHEPRI-DEC-024`, recovery follows Clerk's credential ownership: `R5-02`, `R5-03`, and `R5-04` are deferred; `R5-05` is reframed around live account revalidation, revocation of every Khepri session, and content-free security evidence; `R5-06` is reframed around those consequences, idempotency, disabled/purged refusal, and identity-link integrity. The cross-system window between Clerk credential replacement and Khepri revocation is explicitly accepted only for non-paying private beta and remains unresolved commercially. No recovery migration exists, and no future migration need or absence is settled. The next actionable task is a bounded plan for the reframed Khepri consequence after the provider-backed account design is approved |
