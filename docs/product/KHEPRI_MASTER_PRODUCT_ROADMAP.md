...
| R7 Commercial RRA bridge | MERGED | Commercial analysis bridge, routes, and consent surface merged. **`#231` is closed**: `R7-03`'s live-authorization guards now carry mutation evidence, and each of the three mutants kills exactly one test, so the resolver's gate and the bridge's gate are proven independent rather than redundant. Carries part of `#211`. See section 0.2 |
