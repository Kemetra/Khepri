...
| R4-08 | Add the counterpart locks redemption serializes against: the account row on the disable path, the session row in session-ending writes | R4-01, R1 merged, R3-04 | no | Named statements + dialect compilation tests; `#155`'s concurrency suite re-run green |
| R4-05 | Implement one-time authenticated redemption into exactly one membership | R4-03, R2 merged, R3 actor resolution, R4-08, R4-06, issuance-versus-purge resolved (`R4-01` §8.2), commit-safe session expiry resolved (`R4-01` §8.5) | no | Membership creation |
...
| R4 Invitations | READY_FOR_PLAN | `R4-01` merged (#209). `R4-02`, `R4-03`, `R4-04` and `R4-08` are unblocked. **`R4-05` is blocked on four things** — `R4-08`, `R4-06` (reordered before it, since the purge cascade must precede redemption), and two owner decisions in `R4-01` §8.2 (issuance versus identity purge) and §8.5 (commit-safe session expiry). R2's membership operations are merged and stable, and R3 actor resolution is available. Note for `R4-01`: adding a caller-supplied role (`invite(role=...)`) makes `test_no_role_change_operation_accepts_a_role_from_its_caller` fail by design — that test marks the point where role validation stops being optional, since neither the domain nor `rca_membership_events` validates roles today |
