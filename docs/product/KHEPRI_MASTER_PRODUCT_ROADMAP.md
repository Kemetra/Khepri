...
25. ~~**Implement `D1-02` through `D1-10`.**~~ **Done; the D1 allocation is exhausted.** All nine slices are on `main` in numeric order: `e8ae48d` (`#446`), `2096ab4` (`#447`), `a65df40` (`#448`), `b28b97e` (`#451`), `9f570bc` (`#452`), `79f2d54` (`#454`), `7d91b20` (`#455`), `fe64bd2` (`#457`), `3dd0746` (`#458`). §16's D1 row carries the per-slice SHAs and reads `IN_IMPLEMENTATION` rather than `MERGED`, because `D1-11` remains in §7's table and is unauthorized. The original text is kept below because it records the constraints the slices were built under.
    ...
    **The first critical-path gate is resolved; the remaining owner questions do not block M4:**
    ...
    2. **`U1`'s `RRA-010` journey-adoption reading.** `U1` is `BLOCKED` on it, and `D1-10`'s roadmap row cannot close without `U1-03`/`U1-05`/`U1-06`/`U1-07` — the accessibility-evidence and visual-regression programme `RCA-008` §Exclusions explicitly leaves to them.
    ...
    **`U1` is no longer in the way, and that is `RCA-008`'s doing.** Precondition 3 makes `U1`'s chart grammar, navigation, accessibility and visual-regression tasks **not preconditions** for D1, and lets a slice needing a decision-surface stylesheet rule take it from `RCA-008` §Scope. So these nine slices proceed while `U1` waits on the owner's `RRA-010` journey-adoption reading.
