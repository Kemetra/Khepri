...
| **U1 Design system** | READY_FOR_PLAN | **§15: a program's status is the status of its next actionable task.** `U1-02` is **merged** at `#350` (`9e92e73`) and `U1-04`'s drawer **structure** lands at `#352` (after its plan at `#351`, `b9aeacd`); this row states the tree at that merge, as every §16 row does: the eight `RRA-012` components live as macros in `rra/rendering/templates/_components.html.j2`, both report templates render through the seven data-display ones, and the evidence drawer is proven against `report_api._cited_figure`'s projection but **placed on no page** — a guard test holds that until its supply exists. The next actionable task is `U1-04`'s catalog **supply**, and it is design-only today: `RRA-012`'s Scope note defers it to its own authority because every surface in Scope is a stored artifact, so a catalog read changes the materialization path (three `render_materialized` implementers, three `build_context` callers, `pipeline.py`, both wirings, an Excel renderer with no HTML component). **`RRA-013` allocates that supply** (the PR carrying this row; it states the tree at that merge): the evidence rides on the `ReportBundle` every renderer already receives, so `render_materialized`, `pipeline.py`, `excel.py` and both wirings are untouched — the trace in its mechanism section corrects `RRA-012`'s enumeration, which was right about the call graph and wrong about the shape of the change. Once active, the supply slice needs its bounded plan and RED tests before the row may read `READY_FOR_IMPLEMENTATION`; drawer placement in `_evidence.html.j2` then follows as an `RRA-012` slice, and the citation route's adoption of the bundle's evidence as an `RRA-011` slice. **Journey adoption is unauthorized** — `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename, so that wiring needs an `RRA-010` slice. `U1-03`, `U1-05`, `U1-06`, `U1-07` are excluded from `RRA-012` and remain `BLOCKED`: no active specification governs their files. `U1-01` is documentation of what exists and is unaffected |
