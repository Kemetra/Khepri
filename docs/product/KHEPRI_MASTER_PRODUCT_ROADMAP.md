...
24. **Activate the D1 product-code authority, then implement `D1-02` through `D1-11`. This is now the first incomplete item on the critical path, and it is owner-authored.** No active specification in `governance/registry.yaml` governs the D1 product-code files: `RRA-014` puts "runtime routes, templates, shell assets, workspace persistence" outside its Scope, `RCA-006` excludes "no route, runtime wiring, template, shell asset, public API, migration, cache, background job, renderer, or existing workspace file", and `RCA-007` excludes routes and HTTP responses by `FR-158`, asserted in test. **D1 is made entirely of what all three exclude.** Do not schedule `D1-02` through `D1-11` until the owner merges that specification — the same instruction item 22 recorded holding for `SV1-02`…`SV1-08`, and it held.
    ...
    **One performance obligation travels with this item.** `SV1-08` ledger §3a measures the composed request at 4003 µs p50 with projection at 11.6 µs — one part in 345 — so `D1-09`'s target is source acquisition, and `FR-144` bars caching, pre-aggregation, materialized views and sampling from that path, making any change there governed rather than local. A card that issues three view reads (the note's §5) multiplies acquisition, not projection.
