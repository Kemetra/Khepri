...
| S1-03 | Select one bounded family. **Discharged in substance by `d753e41` + `74e6a19`, not by a slice of its own**: `S1-02` ranked the candidates and `#463` selected rank 2 inline and named it. No standalone selection slice was opened, and the roadmap records what happened rather than inventing an approval event. **Rank 1's shape is named by `KHEPRI-DEC-035`**: row-level security with a non-owner runtime role is the only admissible shape; the cross-package FK and a local CHECK/trigger are refused with reasons. No implementation is authorized until a specification requirement lands (Constitution IV) | S1-02 | Approved slice |
