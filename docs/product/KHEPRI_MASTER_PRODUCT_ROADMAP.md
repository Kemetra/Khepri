...
| G2/G3 Workspace authority | PROPOSED | **`G2-01` is delivered** (`docs/superpowers/specs/2026-09-03-g2-01-retained-data-inventory.md`, measured on `457f276`): 22 tables, two object namespaces, two cookies, one telemetry sink, each with what ends it. **Two findings for the owner before any retention choice**: (F-1) no retention sweeper has a caller on the deployed path — the only one is `khepri.local.cli`, excluded from the wheel — so every horizon, the beta's seven days included, is unenforced in the shipped image; (F-2, corrected in review) on-demand deletion is complete — rows and objects — but `rra_dataset_profiles.document` / `rra_fact_packages.document` have no expiry column and hold customer-derived labels and values as plaintext JSON, unencrypted at rest, until the customer deletes; whether they should be envelope-encrypted like the objects is an `RRA-002` reading. **`G2-02` is on the owner's desk** as the choice sheet OD-1..OD-8 in `docs/platform/proposed-governance/decision-draft-durable-retail-content-retention.md` (`[DEC-RETENTION]`); `G2-03` is that draft's promotion. **`G3-01`..`G3-03` are drafted** as `specification-draft-rca-workspace-history.md` (`[RCA-WORKSPACE]`), activatable only after `[DEC-RETENTION]`. `G3-04` follows both. Status stays `PROPOSED`: nothing is registered |
