...
| **U1 Design system** | READY_FOR_IMPLEMENTATION | **§15: a program's status is the status of its next actionable task, and that task is `U1-02`.** §15 defines `READY_FOR_IMPLEMENTATION` as *"an approved bounded plan and its RED tests exist"*, and both now do: `docs/superpowers/plans/2026-09-01-u1-02-component-layer-plan.md` and `tests/test_u1_02_component_layer.py`, ten failing tests whose FR-101 guard is mutation-verified — it enumerates templates from the directory, and a figure smuggled into `report.pdf.html.j2` was caught by a scan that never named that file. The blocked remainder is recorded in this reason column rather than in the status, because §15 enumerates the permitted values and a compound status is an invented one — the defect `#214` produced. **Active `RRA-012` allocates the governed data-display component layer**: the report and evidence templates, a new component stylesheet under `rra/rendering/`, and the component layer's own bilingual chrome labels. It closes the gap that `RRA-011` merged six catalog routes no template consumed. It is `RRA`, not `RCA`, because the surfaces rendering evidence are `rra/rendering/templates/` — an `RCA` authority over them would contradict `RCA-002`:134 on the day it merged. **`U1-02` is fully authorized. `U1-04` is authorized for the drawer's structure, labelling and keyboard behaviour only** — *supplying* its catalog data is deferred, because every surface in Scope is a stored artifact, so a catalog read means changing the materialization path: a three-implementer `render_materialized` contract, three `build_context` callers, `pipeline.py`, and both wirings, including an Excel renderer that displays no HTML component. That needs its own authority and is the next governance artifact after this one. **Journey adoption is also unauthorized** — `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename or allowlist change, so that wiring needs an `RRA-010` slice. `U1-03` (chart grammar), `U1-05` (navigation and filter bars), `U1-06` (accessibility evidence), and `U1-07` (visual regression) are excluded from `RRA-012` and remain `BLOCKED` for the original reason: no active specification governs their files. `U1-01` is documentation of what exists and is unaffected |
