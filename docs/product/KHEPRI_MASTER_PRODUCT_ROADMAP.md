...
| **D1 Decision workspace** | IN_IMPLEMENTATION | **The allocation is exhausted: `D1-02`…`D1-10` are all on `main`, and `D1-11` — the only task left in §7's table — is unauthorized.** §15's clause is the reason this is not `MERGED`: "a program stays `IN_IMPLEMENTATION` while merged work exists on `main` and a governance question gates the next branch; the gating question is named in the program's row rather than reverting it to `BLOCKED`". The gating question is `D1-11`: `RCA-008` §Retention declines to amend `KHEPRI-DEC-015` §3, so repeat-use telemetry stays prohibited and `D1-11` is unauthorized rather than unscheduled. **Per-slice SHAs (§15 requires a `main` SHA for each):** `D1-02` `e8ae48d` (`#446`), `D1-03` `2096ab4` (`#447`), `D1-04` `a65df40` (`#448`), `D1-05` `b28b97e` (`#451`, superseding the drawer merged at `#450`), `D1-06` `9f570bc` (`#452`, a plan slice: S-7 already shipped and S-8 is owner-blocked, so it had no deliverable), `D1-07` `79f2d54` (`#454`), `D1-08` `7d91b20` (`#455`), `D1-09` `fe64bd2` (`#457`), `D1-10` `3dd0746` (`#458`). The allocation plan is `docs/superpowers/plans/2026-09-10-d1-02-10-decision-workspace-allocation-plan.md`; each slice's evidence ledger sits beside it. **Three owner-authored items are carried forward, and the first is a milestone consequence rather than a loose end.** (1) **The successor composition artifact for Period Comparison.** `RCA-008` does not claim the two-population binding, so `D1-03` still ships `FR-170`'s asserted absence and **D1 alone does not satisfy M4's "compare governed periods"** — item 25 states this and merging D1 does not change it. Binding the source is a composition-root change in a file `RCA-007` §Scope already governs, and two active specifications over one file is the ambiguity this repository fails closed on. (2) **The export reading in `D1-08` §Exclusions** — whether "no cross-organization access, sharing, or export" reaches "export". Print shipped; export is undecided. (3) **`D1-11`**, above. **One row this program did not close, recorded so it is not read as closed.** `D1-10`'s own task row in §7 is wider than `RCA-008` §Exclusions authorizes: the accessibility-evidence and visual-regression halves remain `U1-03`/`U1-05`/`U1-06`/`U1-07`'s and need their own authority, and `U1` is itself blocked on the owner's `RRA-010` journey-adoption reading. Parity, refusal states and cross-organization isolation are done; RTL and mobile shipped as parity evidence, not as a layout programme. **Why this differs from `SV1`'s `MERGED`.** `SV1` reached `MERGED` with its composition slice shipped (`RCA-007` at `2337cd4`); D1's composition artifact does not exist, so the analogue does not carry. **Historical note, kept because it records how the status moved.** It read `READY_FOR_PLAN` at `#444`: "the authority merged at `2734886`, so §15 moves this row off `BLOCKED`: the program's next actionable task is `D1-02`, and it may start." Active `RCA-008` (`FR-159`–`FR-171`) governs `src/khepri/rca/workspace/`, `shell_api.py`, `shell_templates/` and `src/khepri/runtime/shell_assets/`, for the D1 surfaces only. Before that it read `BLOCKED`, naming the absent product-code authority as the dependency; `D1-01` (`8e6dc94`, `#440`) was the information-architecture note that could proceed meanwhile |
...
25. ~~**Implement `D1-02` through `D1-10`.**~~ **Done, and the critical-path queue is exhausted with it.** All nine slices are on `main` in numeric order: `e8ae48d` (`#446`), `2096ab4` (`#447`), `a65df40` (`#448`), `b28b97e` (`#451`), `9f570bc` (`#452`), `79f2d54` (`#454`), `7d91b20` (`#455`), `fe64bd2` (`#457`), `3dd0746` (`#458`). §16's D1 row carries the per-slice SHAs and reads `IN_IMPLEMENTATION` rather than `MERGED`, because `D1-11` remains in §7's table and is unauthorized. The original text is kept below because it records the constraints the slices were built under.

    **There is no item 26, and that is a finding rather than an omission.** This was the last item on the critical path, so **the queue below is empty and the next move is owner-authored in every case.** A reader who takes a struck item 25 for "nothing left to do" has it backwards: what remains is not schedulable by a plan, which is precisely why it is not listed as an item.

    **The three things that gate a new critical path, in the order they bind:**

    1. **The successor composition artifact for Period Comparison.** Named in item 25's own M4 paragraph below and unchanged by D1 merging: `RCA-008` does not claim the two-population binding, so **D1 alone does not satisfy M4's "compare governed periods."** Until it exists, `D1-03` ships `FR-170`'s asserted absence and the Compare surface is held open rather than built. This is the one item that blocks a *milestone* rather than a task. **It is now briefed rather than merely named**: `108272a` (`#464`) carries `docs/superpowers/plans/2026-09-15-period-comparison-composition-brief.md`, which verified each claim against the tree, quoted the three clauses that jointly create the gap, and reduced the blocker to two owner decisions — which artifact shape (amend `RCA-007`, a new `RCA-009` successor, or a `KHEPRI-DEC-*` re-reading), and whether `M4` waits. **The brief deliberately records no recommendation and adds no registry row**, because authoring the artifact silently elects the first branch of the second decision. It makes the decision a signature rather than an investigation; it does not move the gate.
    ...
    **The M4 consequence, stated rather than buried.** `RCA-008` does not claim the two-population binding, so **D1 alone does not satisfy M4's "compare governed periods."** A successor composition artifact is the precondition, it is owner-authored, and the alternative is to accept a D1 milestone without Compare. This is the one thing between the frontier and M4 that no plan here can schedule.
