...
| C1-05 | Add comparison HTML/PDF/Excel surfaces and reconciliation | C1-03 | C1-04 | Deliverables — **NEXT, and it is not the rendering slice its title describes.** The renderers do not accept facts: `html.py`, `pdf.py` and `excel.py` all take `ReportBundle`, whose `figures` are `CitedFigure` — text already rendered for every language, so that a surface can only reproduce it. A `CrossVersionFact` cannot reach any of them, because it is a sibling of `Fact` and `bundle.py`'s retained union is `Fact \| FactSeries \| FactComparison` at seven sites. **So the work is a carrier seam in `bundle.py`, not an edit to 5,047 lines of renderer**: widen that union, and add a `_analysis_figure`-shaped converter that pre-renders both languages. Verify before starting whether that seam belongs here or to `C1-06`, which owns orchestration — the title says surfaces, the type signatures say plumbing. Two constraints: `RRA-006` requires the bundle publish together or not at all, so a partial surface is not shippable and the slice cannot be split HTML-first; and nothing in `src/` calls `C1-01`–`C1-04` yet, so this is where the chain either acquires a production caller or adds a fifth module nothing calls — the shape `W1-04b` was opened for. State which, rather than leaving it implied |
