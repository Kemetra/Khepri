...
| **U1 Design system** | READY_FOR_IMPLEMENTATION | **§15: a program's status is the status of its next actionable task, and that task is `U1-04`'s drawer placement under `RRA-012`**, which now has its bounded plan and RED tests: `docs/superpowers/plans/2026-09-03-u1-04-drawer-placement-plan.md` and `tests/test_u1_04_drawer_placement.py` (six strict-xfail tests on the rendered evidence, business and printed documents, plus one premise guard). **Merged so far:** `U1-02` (`#350`), `U1-04`'s drawer structure (`#352`), active `RRA-013` (`#353`), its plan (`#354`) and its supply (`#355`, `ca6a609`): the bundle carries one evidence record per citation and coverage in its identity (`BUNDLE_VERSION` v8), the audit context carries `evidence` and `coverage`, the print context sets `evidence_open`. The placement slice puts the drawer beside each evidence figure row, passes coverage as its own argument, opens it on paper, and deletes the placement guard. After it, the citation route's adoption of the bundle's evidence is an `RRA-011` slice. **Journey adoption is unauthorized** — `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename, so that wiring needs an `RRA-010` slice. `U1-03`, `U1-05`, `U1-06`, `U1-07` are excluded from `RRA-012` and remain `BLOCKED`: no active specification governs their files. `U1-01` is documentation of what exists and is unaffected |
