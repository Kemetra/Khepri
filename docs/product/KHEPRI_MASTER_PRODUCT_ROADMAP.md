...
| **U1 Design system** | READY_FOR_PLAN | **§15: a program's status is the status of its next actionable task.** Everything `RRA-012` and `RRA-013` authorize is **merged** (2026-09-03): `U1-02`'s component layer (`#350`), `U1-04`'s drawer structure (`#352`), active `RRA-013` (`#353`) with its plan and supply (`#354`, `#355`), the drawer placed beside every evidence figure row (`#356`, `#358`), the pagination-test correction that placement required (`#359`), and the `RRA-011` follow-on in which the citation route reads the bundle's evidence and its duplicate projection is deleted (`#360`, `37455a5`). What remains of `U1` is **authority, not code**: journey adoption of the component layer needs an `RRA-010` amendment — `journey/templates/base.html.j2` loads only `/beta/assets/journey.css` and `RRA-010`:88 excludes a new asset filename — and `U1-03`, `U1-05`, `U1-06`, `U1-07` have no active specification governing their files. The next actionable task is therefore drafting that `RRA-010` amendment, which is design and may proceed; no slice may implement it until the amendment is active — the named blocking dependency, so the program reads `READY_FOR_PLAN` and not `BLOCKED`. `U1-01` is documentation of what exists and is unaffected |
