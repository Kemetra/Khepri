...
28. ~~**The `RRA-015` `report.css` accessibility-floor fix.**~~ **Done — all five floors hold and every strict marker is deleted, which `FR-193` makes the completion test.** Two of the three recorded reasons were wrong: `min-block-size` does nothing to a non-replaced inline box, so the anchors needed `inline-flex` before the floor could apply, the Arabic surface also failed on width, and the skip link was a third failing tab stop no reason named; and the reflow was **not** the figures table, which its own `.scroller` contains, but `dl.provenance`, whose `minmax(12rem, auto)` term column doubles past its box at 200% text. **A plausible subject named in an `xfail` reason is a hypothesis, not a diagnosis.** Slice 9a measured `FR-189`'s floors and, as its allocation plan requires, **recorded** five failures rather than fixing them: an evidence slice that edits a stylesheet to make its own assertion pass has left its scope. All five are live as `xfail(strict=True)` and all confirmed in CI. **This queue previously said “no new authority is needed”, and that was wrong** — `RRA-015` §Scope governed `report.css` for *chart presentation rules only*, §Successor reading item 3 said “no slice under this document changes a component rule”, and `_components.html.j2` was named by neither `RRA-015` nor `RRA-012` except by directory. All five failures sit in component rules, so the document could measure floors it could not satisfy. **A bounded amendment closes exactly that gap**, adding `FR-193` and the §Successor reading item 3a carve-out for these five recorded failures alone; `RRA-012` keeps the component layer generally. **That amendment merged at `b51494f` (`#501`), so the authority is in place.** They are the 44px target floor on the report surface's “On this page” anchors, rendering at 21px in both languages (`report.css`, which declares no minimum-target rule at all); the refusal panel's floor in `_components.html.j2`; and the evidence surface's ~184px horizontal overflow at a 390px viewport under 200% text scaling, in both languages. The strict markers fail the day each floor is met, so the fix slice is self-verifying. **Proving the floors under the shipped Arabic faces needs an HTTP origin so `@font-face` fetches** — `tests/test_rca011_shell_font_load.py` is the worked example; a `set_content` page has no origin and measures whatever the host has installed.
