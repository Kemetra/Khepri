...
| G4-02 | Activate RRA comparison-fact authority | G4-01 | RRA specification — **drafted**, `RRA-008` §Two-population comparison, `rra008.crossversion.v1`; governing only if the owner merges it |
| G4-03 | Activate RCA comparison orchestration authority | G4-01 | RCA specification — **drafted**, `RCA-005` §Comparison orchestration `FR-130`–`FR-133`, comparison **derived at read time**; governing only if the owner merges it |
| G4-04 | Freeze versioned input/output, compatibility, filter, and evidence contracts | G4-02/03 | Contract baseline — **drafted**, `RRA-008` §Frozen contracts + `RCA-005` §Frozen contracts, frozen by reference; filter half frozen at **identity** rather than an enumeration; governing only if the owner merges it |
...
| C1-02 | Detect incompatible semantics, currency, population, coverage, mapping drift, and versions | C1-01 | no | Fail-closed compatibility contract — **MERGED** `3890d8b` (`#402`): `analysis/compatibility.py`, `D-1`–`D-5` as one ordered table returning the first cause |
...
| C1-05 | Add comparison HTML/PDF/Excel surfaces and reconciliation | C1-03 | C1-04 | Deliverables — **NEXT, as a sibling bundle under `RRA-006` §Two-population bundle** (the additive amendment this row's PR carries). `#406` said the work was widening `bundle.py`'s retained union; **that was wrong twice over**: the union is two sites, not seven, and it must not be widened at all, because `RCA-005` §Comparison retention says a comparison is retained as no content class, and `_cited_evidence`'s `record is None` branch (“derived by a family during assembly and retained by nothing”) is already the governed path. Three facts fix the shape: `BundleIdentity.of(package)` digests one source digest and a comparison has two; `ORDERED_SECTIONS` is a closed five-tuple whose `comparison` entry is period comparison under `rra008.comparison.v2`; and no active specification authorized a cross-version surface before this amendment (`RRA-008` §Not authorized here excludes it, `RRA-006` never named it). So: a `CrossVersionBundle` beside `ReportBundle`, identity over the ordered pair, versioned `rra006.crossversion.bundle.v1`, rendered through the existing three surfaces and reconciled by the existing rules, **rendered on request and stored nowhere** (`KHEPRI-DEC-033` §2 retains report artifacts with a run, and a comparison has none). One tension to carry, not resolve here: `RRA-008` §Two-population comparison says the fact carries “the same `Fact` shape — no new type” while §Composite provenance leaves the carrier to `C1-03`, which chose a sibling type; two clauses pointing different ways is an owner reading, and `C1-08` should test whichever the owner takes |
...
## 16. Recommended current status at `c98e946`
...
17. **`W1-01` onward — begin durable workspace/history implementation. This is now the first incomplete item on the critical path.** Order is fixed by `G3-04` §1-2: contracts (`W1-01`), persistence and the one-head migration (`W1-02`), the tombstone allowlist (`W1-03`), services (`W1-04`), then surfaces in `FR-049` link order (`W1-05`, split into Overview/Data then Analyses), Analysis detail and the Passport (`W1-06`), the Methodology Change Notice (`W1-08`), then deletion and the retention sweep (`W1-07`), then isolation hardening (`W1-10`). Build order is deliberately not numeric: `W1-07` introduces the first destructive path and lands after the read surfaces it can affect. **`W1-07` carries `KHEPRI-DEC-033` §5** and is the only slice that can discharge it: until its sweep ships with a caller in the built wheel, every retention horizon is unenforced and no surface may say content expires automatically.
    ...
    **Status at 2026-09-06.** That obligation is **discharged**: `W1-07b` (`89796bd`, `#384`) shipped the sweep with a caller in the built wheel. Ten slices are merged — `W1-01`…`W1-08`, `W1-04b`, `W1-10` — and the order above held, with deletion landing after the read surfaces it can affect. **`W1-09` is the remainder and was never in `G3-04`**, which allocated ten IDs without one for it: `RCA-005` named no requirement for pins or recent activity, and its Exclusions barred both halves. `#386` (`1d8c5de`) closed that by authoring `KHEPRI-DEC-034` as a **new** Constitution VII decision rather than an amendment to `KHEPRI-DEC-015` §3 — which is why `W1-11` and `R8-08` stay excluded and item 10 above is unchanged by it. `W1-09` builds last, after every surface a pin can point at. **Done** — `cf607fd` (`#390`), 2026-09-06. **This item is closed and the M3 workspace chain is complete.** The next critical-path work is `G4/C1` comparison, which §16 records as depending on `W1` and on new split authority the owner must author.
...
20. **`G4-02` is drafted and proposed (`#397`+); `G4-03` remains open.** Both activate the split RRA comparison-fact and RCA orchestration authority. **`G4-02`**: `RRA-008` gains §Two-population comparison — a fifth family reading exactly two `RRA-004` populations under governed version `rra008.crossversion.v1`, the six `G4-01` predicates as refusal causes, P-1's compare-as-stated rule with its natural-calendar-length allowance and its refusal of the prefix concession across versions, and bilingual parity for every cause it adds. Additive: the four single-population families, `RRA-004`, and the registry are untouched. **Approval is the owner's merge**, per Constitution II — a drafted amendment on a branch is a proposal and nothing more. **`G4-03`**: `RCA-005` gains §Comparison orchestration — `FR-130` (two versions in one organization, uniform refusal, pairwise), `FR-131` (refuse on `RRA-008`'s predicates and on an unordered pair, no partial figure), `FR-132` (reachable from Analyses naming both versions, each linking to `FR-119`'s Passport), `FR-133` (one content-free audit event per request and per refusal under `FR-125`'s shape). **The owner question is answered: a comparison is derived at read time and creates no retained content class**, on Constitution VII's least-data default — the authority item 21 restored, which is what made the question answerable. It narrows rather than deletes its own comparison exclusion, keeping the reason (a workspace surface does not calculate) and admitting orchestration only. **It also adds `RRA-008` to `RCA-005`'s `depends_on`**, correcting a pre-existing omission: line 84 already required a run to name the `RRA-008` pipeline while the registry did not declare it, and `FR-131` would have deepened an undeclared coupling. **`G4-04`**: both specifications gain a §Frozen contracts section naming the four halves **by reference** rather than restating them, so the two documents cannot drift — input (ordered subject/baseline pair), output (`Fact` under `rra008.crossversion.v1`, `subject - baseline`), compatibility (`D-1`–`D-6` plus §Period rule), and evidence (both identifiers, bases, manifests, and the order). **The filter half is frozen at identity, not at an enumeration**: `D-5` already requires identical event-kind and status filters across the pair, so the admissible set is exactly whatever `RRA-003`/`RRA-004` admit for one population and any difference refuses. No filter vocabulary is invented — there was none to freeze, and defining one would have widened authority. `C1-08`'s "unsupported-filter" case therefore means a filter `RRA-004` does not admit, or a pair whose filters differ. **`C1-01` is next and is a different kind of work**: eight product code slices under Constitution IV and this repository's TDD rules, including the bilingual customer strings `G4-02` deliberately left to the code slice that raises each refusal. Owner-authored; this queue does not schedule them.** `G4-01`'s scope note is the input: it asks for a **pair admission** rather than a new calculation, because `RRA-008` already ships period comparison from "one compatible `RRA-004` population" and `RCA-005` reserves "comparison run" while authorizing none of it. The note proposes two admitted use cases (same lineage one period later; the same period re-issued after a correction, at identical aggregate scope and admitted store set), **refuses** a cross-mapping/cross-formula delta as a number with no interpretation, and defers N-way. **`#394`'s review corrected two conflicts with `RRA-008` before merge**: a first draft's period rule would have refused natural calendar length differences that `RRA-008` explicitly permits (February against March), and its UC-2 admitted a cross-store pair that `RRA-008` refuses as "store-mismatched" — a breakdown within one population is `RRA-004`'s, not `C1`'s. Its six-predicate gate — one organization scope, identical mapping, formula and package versions, same currency and population, complete `RRA-003` coverage — is `C1-02`'s fail-closed contract enumerated. **Prepared, not activated**, by `docs/superpowers/plans/2026-09-07-g4-02-g4-03-preparation.md` — which states the sections `G4-02` adds to `RRA-008` (a two-population fact under its own governed formula version, the six predicates as refusal rules, and P-1 with its explicit calendar-length carve-out), and the four requirements `G4-03` adds to `RCA-005` as `FR-130`–`FR-133`. `G4-03` is the harder half because it must **narrow one of `RCA-005`'s own exclusions** — the comparison deferral `G4-01` respected — keeping the reason (a workspace surface does not calculate) while admitting orchestration. **One correction to `G4-01`, which merged with it wrong.** That note said `G4-02` should add the `Fact` field recording `COMPARISON_FORMULA_VERSION`; **both halves were already discharged** — `Fact.formula_version` exists (`facts.py:267`, required when `APP-014` amended `RRA-004`) and `comparison.py:603` sets it, landed in `#81`. `G4-01` cited `comparison.py`'s module docstring, which still claims "nothing does today" and is stale by ~300 PRs. What remains is a `C1-03` **test** obligation, not an amendment: `fact_identity()` hashes `COMPARISON_FORMULA_VERSION` while the field defaults to `rra004.formula.v2`, so a second construction site omitting the argument would stamp a mismatched version and nothing would refuse it. **The open owner question stands**: whether a completed comparison is retained content or derived at read time. `KHEPRI-DEC-033` §2 has no row for it; the note recommends derived-at-read-time on the least-data default — **the authority item 21 has now restored** (Article VII, Constitution `2.1.0`), so that premise exists and `G4-03` may cite it. Ordering is unchanged: item 21 → `G4-02` → `G4-03` → `G4-04` → `C1`.
