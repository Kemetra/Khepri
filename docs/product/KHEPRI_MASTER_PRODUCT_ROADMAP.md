...
milestone now under review first resolves that prerequisite provider-neutrally: account and link
pre-provisioning share one transaction, and final-owner capability is a local verifier or an
external identity link. The persistence probe found the existing nullable verifier columns and
external-identity table sufficient, so that capability needs no schema change. The companion
...
| R3 Authentication sessions | IN_REVIEW | `R3-01`…`R3-10` are merged, and active `KHEPRI-DEC-024` provisionally admits Clerk for non-paying private beta. The Clerk private-beta milestone is implemented on this branch and under review: atomic provider-neutral external account/link pre-provisioning, external-capable final-owner protection, the contained Clerk adapter, production session handoff, hard-stop revocation, and signed-token RCA→RRA evidence. The persistence probe found no external-account migration necessary; existing nullable verifier columns and `rca_external_identities` express the model. `R3-11` is not `MERGED` until the owner merges this branch |
...
| R5 Recovery | IN_REVIEW | `R5-01` remains the design for a future Khepri-credential path. Under active `KHEPRI-DEC-024`, `R5-02`, `R5-03`, and `R5-04` remain deferred; the reframed `R5-05`/`R5-06` consequence is implemented on this branch and under review: verified-link resolution, live account revalidation, all-Khepri-session revocation, idempotent content-free evidence, disabled/purged refusal, identity-link integrity, and twelve-month evidence retention. Migration `20260821_0019` adds only that evidence table. The accepted cross-system recovery window remains a non-paying private-beta residual and a commercial hard stop; this row is not `MERGED` until the owner merges the branch |
