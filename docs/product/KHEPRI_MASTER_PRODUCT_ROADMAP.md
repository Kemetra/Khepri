...
| **T1 Trust/catalog** | IN_IMPLEMENTATION | **Bounded authority now exists**: active `RRA-011` allocates the metric, population, and reason catalog, its bilingual vocabulary, and its evidence surfaces, governing `T1-01` through `T1-05` and `T1-08`. `T1-06` lineage is excluded and may complete during early `M3`; `T1-07` trust telemetry is excluded and **blocked** on the same `KHEPRI-DEC-015` prohibition as `R8-08` (`KHEPRI-DEC-031` §5). `IN_IMPLEMENTATION` per §15: `T1-01` through `T1-04` merged at `#334`, `#338` and `#340`, and `T1-05` with the `T1-08` evidence remainder followed. The row read `READY_FOR_PLAN` on the ground that no slice had merged, which three had |
...
15. ~~**M2 acceptance, in its local-only form.**~~ **Done** — all four `KHEPRI-DEC-031` §7 conditions hold on `bc96a65`, and §6's `F5` requirement was met at `#334`. Condition 1 failed at `#342` because `definitions.py` had no production consumer; `#343` merged the two read routes that ledger named, and `docs/superpowers/plans/2026-08-30-m2-condition-1-remeasured.md` re-measures it. The bilingual rehearsal ran inside `khepri-staging-web` against the merged staging stack: journey to `rra004.package.v3`, then every catalog surface in both languages — quality `200` in each at 3,598/4,692 bytes, metric names `Revenue`/`الإيرادات`, unknown code `404`, and **30 of 30 displayed citations resolving**. `M2` is reached **in its local-only form**, with its hosted-environment and activation-telemetry clauses **recorded as unmet rather than checked**. Internal only: `KHEPRI-DEC-031` §2 admits no external participant, and external alpha needs both the hosted environment (item 11) and the amendment item 10 names.
