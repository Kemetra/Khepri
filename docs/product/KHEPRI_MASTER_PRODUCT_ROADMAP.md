...
| C1-03 | Build immutable RRA comparison fact package | C1-02 | RCA API after freeze | Comparison facts — **NEXT, and it opens with a design decision the specification deliberately left to it.** `RRA-008` §Composite provenance requires four new disclosures per fact (both versions' identifiers, both cited retained bases, both coverage-manifest identities, and the operand order) while stating that `Fact.inputs` "keeps its existing meaning and is not repurposed" and the `RRA-004` `Fact` type "is not modified" — so **the carrier is this slice's to choose**: a wrapper type, a package-level field, or a parallel record. `C1-01` and `C1-02` had their answers fully determined by frozen text; this one does not, so open it with the carrier question stated rather than assumed. Two constraints to carry in: **operand order is part of fact identity**, so the reversed pair is a distinct fact with a distinct identity rather than a negated value, and reruns are byte-equivalent only for the same ordered pair; and `C1-03` is the slice that inherits `comparison.py`'s `COMPARISON_FORMULA_VERSION` obligation as a **test** (see item 20) — `fact_identity()` hashes it while `Fact.formula_version` defaults to `rra004.formula.v2`, so a construction site omitting the argument stamps a mismatched version and nothing refuses it. One smaller note: `compatibility.py`'s `_PREDICATES` table types its comparison function as `object` with two `# type: ignore` comments; nothing gates on mypy here, but a third comparison kind is where that annotation would go wrong silently |
